# Alps: Prefetch and In-situ Reshape Innovation Plan

## Scope and evaluation rule

HexKL remains the compute baseline. Alps should optimize how HexKL-ready
...
Alps path. The active scope is prefetch, DMA, in-situ layout production,
...
experiment but must not be enabled in the cumulative Alps configuration or
...
| Row | Meaning |
|---|---|
| HVX | Hexagon-MLIR with HexKL and all Alps features disabled |
| HexKL | the same model/input with HexKL enabled and Alps disabled |
| HexKL + cumulative Alps | HexKL plus only the innovation items that have passed the ordered gate |
...
- `alps.layout_value_id`;
- `alps.layout_source_kind`;
- `alps.layout_site_occurrences`;
- `alps.layout_estimated_executions`; and
...
- Alps enum attributes now have textual parse/print support;
- PrefetchInsert, V-DAE, and LayoutOpsElimination are registered in
  `linalg-hexagon-opt`; and
- `alps_layout_value_analysis.mlir` verifies identity grouping and static
...
| HexKL + cumulative Alps | 1689.332 ms | top-1 check passed, max abs 0.0239 |
...
- `native`: retain the HexKL `rm_to_wh` transform and avoid constructing a
  one-use synchronous Alps wrapper;
...
Every decided site records `alps.transform_mode`,
`alps.transform_score`, `alps.transform_useful_tiles`, and
`alps.transform_outer_reuse`. Persistent candidates additionally record
`alps.persistent_candidate`. Function-level native/sync/async/persistent
counts and `[TransformCostModel]` diagnostics make it possible to prove which
paths a model actually exercised.

`alps_transform_cost_model.mlir` covers a short native weight site, a
...
create one-use Alps operations, item-1 layout analysis now observes the
...
remain on the emitted Alps path. Weight layout propagation should remain
...
- `alps.layout_carried_from_producer`;
- `alps.layout_carried_view_depth`.

The function records `alps.layout_carried_sites` and
`alps.layout_carried_views`, and emits a `[LayoutCarryFusion]` summary.
`alps_layout_carry_fusion.mlir` verifies both a fused static-contiguous
...
`__alps_wh_cache_set_context(context, generation)` selects the active
generation. The embedding runtime must increment generation whenever weights
belonging to the same context may change.
`__alps_wh_cache_invalidate(context, generation)` explicitly invalidates
that generation, and `__alps_wh_cache_stats()` exposes packed hit/miss
...
| HexKL + Alps items 1–4 | 1612.315 ms warm average | top-1 match, max abs 0.0239 |
...
weight sites. It is controlled by `enableAlpsTwoDimPipeline` /
`--enable-alps-two-dim-pipeline` and remains off by default.
...
The new
`qcom_hexagon_backend/test/Conversion/LinalgToLLVM/alps_two_dim_pipeline.mlir`
...
| HexKL + Alps items 1–5 | 1586.580 ms warm average | top-1 match, max abs 0.0239 |
...
It is controlled by `enableAlpsVtcmColoring` /
`--enable-alps-vtcm-coloring`, is disabled by default, and is enabled only
...
`alps.vtcm_legacy_peak_bytes`,
`alps.vtcm_colored_peak_bytes`,
`alps.vtcm_saved_peak_bytes`, and
`alps.vtcm_colored_sites` attributes so that the allocation result is
...
lit -sv \
  triton/build/cmake.linux-x86_64-cpython-3.11/third_party/qcom_hexagon_backend/test/Transforms/alps-vtcm-lifetime-coloring.mlir \
  ...
  triton/build/cmake.linux-x86_64-cpython-3.11/third_party/qcom_hexagon_backend/test/Conversion/LinalgToLLVM/alps_two_dim_pipeline.mlir \
  triton/build/cmake.linux-x86_64-cpython-3.11/third_party/qcom_hexagon_backend/test/Conversion/LinalgToLLVM/alps_persistent_wh_cache.mlir
```
...
ANDROID_SERIAL=49d1c7b2 \
  bash scripts/run_alps_model_ablation.sh \
...
| HexKL + Alps items 1–6 | 1598.756 ms warm average | top-1 match, max abs 0.0239 |
...
`--enable-alps-kv-cache-prefetch`; the logical page size is controlled by
`alpsKvCachePageTokens` /
`--alps-kv-cache-page-tokens` and defaults to 32 tokens. Both are opt-in,
...
- `alps.kv_prefetch_sites`;
- `alps.kv_prefetch_hints`;
- `alps.kv_prefetch_pages`;
- `alps.kv_prefetch_bytes`; and
- `alps.kv_direct_layout_sites`.
...
failure. Copying only the two Alps semantic attributes fixed the baseline.
...
lit -sv \
  ...
  triton/build/cmake.linux-x86_64-cpython-3.11/third_party/qcom_hexagon_backend/test/Transforms/alps-kv-cache-prefetch.mlir \
  triton/build/cmake.linux-x86_64-cpython-3.11/third_party/qcom_hexagon_backend/test/Transforms/alps-vtcm-lifetime-coloring.mlir \
  triton/build/cmake.linux-x86_64-cpython-3.11/third_party/qcom_hexagon_backend/test/Conversion/LinalgToLLVM/alps_two_dim_pipeline.mlir \
  triton/build/cmake.linux-x86_64-cpython-3.11/third_party/qcom_hexagon_backend/test/Conversion/LinalgToLLVM/alps_persistent_wh_cache.mlir
```
...
| HexKL + Alps items 1–7 | 599.969 ms warm average | top-1 match, max abs 0.0231 |
...
production. It is controlled by `enableAlpsDequantReshape` /
`--enable-alps-dequant-reshape`. The flag is disabled by default and is
not part of the formal items-1-through-7 cumulative row.

The implementation is end to end:

1. The Alps dialect has a distinct `HMXWeightDequantI8` layout kind.
...
triton/build/cmake.linux-x86_64-cpython-3.11/third_party/qcom_hexagon_backend/bin/linalg-hexagon-opt \
  qcom_hexagon_backend/test/Transforms/alps-dequant-reshape.mlir \
...
|---|---:|---|---|
| HexKL + Alps items 1–7 | 597.726 ms warm average | top-1 match, max abs 0.0231 | not enabled |
| HexKL + Alps items 1–8 (experimental) | 670.129 ms warm average | top-1 match, max abs 0.0233 | cold 546 hits / 606 misses; cumulative 2682 hits / 1926 misses |
...
item 8 must not enter the formal `HexKL + cumulative Alps` row.
...
not part of the forward Alps roadmap.
...
The following facts materially constrain Alps design:

| V73 hardware fact | Consequence for Alps |
|---|---|
...
- `alps_l2fetch_weight_tile()` issues 32 separate 64-byte commands for one
  32-by-32 FP16 tile. On V73, each new command can halt the preceding active
  command, so most rows may never become resident.
- `alps_l2fetch()` uses 32 KB chunks. This is legal for the extended
...
feature ideas. They form one extension of Alps from a prefetch insertion
...
Existing Alps supplies the starting mechanisms: layout-aware prefetch,
...
| Unified decision | Existing Alps instance | V73 mechanisms that complete it |
|---|---|---|
...
HVX, HexKL, and HexKL plus the admitted Alps stages. A mechanism that
...
The first M1 implementation is present in `AlpsRuntime.c` and is deliberately
nonblocking:
...
```bash
CCACHE_DIR=/tmp/alps-ccache CCACHE_TEMPDIR=/tmp \
  ninja -C triton/build/cmake.linux-x86_64-cpython-3.11

triton/build/cmake.linux-x86_64-cpython-3.11/bin/llvm-lit -v \
  qcom_hexagon_backend/test/Conversion/LinalgToLLVM/alps-kv-cache-prefetch.mlir \
  qcom_hexagon_backend/test/Conversion/LinalgToLLVM/alps_two_dim_pipeline.mlir \
  qcom_hexagon_backend/test/Conversion/LinalgToLLVM/alps_persistent_wh_cache.mlir

python -m py_compile benchmark_models/run_gpt2lmheadmodel.py
bash -n scripts/run_alps_model_ablation.sh
git diff --check
```
...
ANDROID_SERIAL=49d1c7b2 \
  bash scripts/run_alps_model_ablation.sh \
    ...
    --output-dir \
      benchmark_models/results/alps_m1_gpt2_full_f32_seq32
```
...
currently always zero in the Alps weight path. They should become explicit
...
2. HexKL with all Alps features disabled; and
3. HexKL plus only the non-quantized Alps stages that have passed their
   ordered gates.
...
## Movement-amortization regions: the next Alps design layer
...
This does not break the Alps story. It clarifies its next abstraction:
...
`enableAlpsWeightStationary`. For a static FP16 projection
...
`enableAlpsActivationMulticast`. It recognizes two static tensor MatMuls
that:
...
Alps algorithm and the torch-mlir model input are not intrinsically
Triton-dependent.
...
- Therefore the local source edits in `triton/python/src/main.cc` and the two
  `triton_shared` conversion files are not Alps item-5 changes. They are
  compatibility/lowering fixes for the Triton frontend path and remain outside
  the Alps commit scope.
...
| layout-aware Alps | 1689.758 ms | top-1 match, max abs 0.0239 |
...
`scripts/run_alps_model_ablation.sh` prevents future unbounded waits.
...
### Debug-model result on `alps-2x-improvement`
...
ANDROID_SERIAL=49d1c7b2 scripts/run_debug_matrix.sh \
  --runtime-root /tmp/alps-2x-improvement \
  --seq-len 32 \
  --timeout 600 \
  --output-dir /tmp/alps-unfinished-debug-seq32 \
...
`alps-2x-improvement` tag while the runner sources came from this
...
spectrogram patches.  Together they test whether the Alps abstraction
...
ANDROID_SERIAL=49d1c7b2 scripts/run_debug_matrix.sh \
  ...
  --output-dir /tmp/alps-new-candidates-debug \
...
ANDROID_SERIAL=49d1c7b2 scripts/run_debug_matrix.sh \
  ...
  --output-dir /tmp/alps-new-candidates-group2 \
...
ANDROID_SERIAL=49d1c7b2 scripts/run_debug_matrix.sh \
  ...
  --output-dir /tmp/alps-new-candidates-group3 \
...
Alps-only regression, but no backend is correct.  It is retained as a
...
ANDROID_SERIAL=49d1c7b2 scripts/run_debug_matrix.sh \
  ...
  --output-dir /tmp/alps-new-candidates-group4 \
...
compiler with the runtime from the `alps-2x-improvement` worktree:
...
ANDROID_SERIAL=49d1c7b2 scripts/run_debug_matrix.sh \
  --runtime-root "$PWD" \
  --timeout 240 \
  --output-dir /tmp/alps-blocker-fixes \
...
functions.  No Alps operation then pulled `AlpsRuntime.bc` into the
kernel object, leaving the reporting symbols unresolved.  DSP logcat showed:

```text
undefined symbol __alps_wh_cache_set_context
undefined symbol __alps_wh_cache_stats
undefined symbol __alps_l2_scheduler_counts
...
ANDROID_SERIAL=49d1c7b2 scripts/run_debug_matrix.sh \
  --runtime-root "$PWD" \
  --timeout 240 \
  --output-dir /tmp/alps-kv-applicability-correction \
...
Alps's scope and produces real gains, the cumulative policy restores it
...
ANDROID_SERIAL=49d1c7b2 scripts/run_debug_matrix.sh \
  --runtime-root "$PWD" \
  --timeout 300 \
  --output-dir /tmp/alps-attention-kv-restored \
...
ANDROID_SERIAL=49d1c7b2 scripts/run_debug_matrix.sh \
  --runtime-root "$PWD" \
  --timeout 300 \
  --output-dir /tmp/alps-fill-15-group1 \
  unispeech-base sew-base sd_unet_crossattn

ANDROID_SERIAL=49d1c7b2 scripts/run_debug_matrix.sh \
  --runtime-root "$PWD" \
  --timeout 300 \
  --output-dir /tmp/alps-fill-15-group2 \
...
1. Alps's full DINOv2 4.2739x gain is real and correctness-valid relative
...
`/tmp/alps-baselines/dinov2-small-full-qnn` and are not committed.
...
1. The existing `run_sd_unet_debug.py` is deployable at about 105 ms, but it
   ...
   denoiser, and has no Alps attention opportunity.
...
ANDROID_SERIAL=49d1c7b2 scripts/run_debug_matrix.sh \
  --runtime-root "$PWD" \
  --timeout 180 \
  --output-dir /tmp/alps-sd-unet-bounded-fix \
  sd_unet_crossattn
```
...
HexKL+Alps configurations reach device execution and pass the numerical
comparison.
...
| HVX | 173.867 ms |
| HexKL | 174.867 ms |
| HexKL + Alps items 1-7 | 60.337 ms |
...
`/tmp/alps-baselines/dinov2-debug-qnn` by default.  The script uses:
...
`/tmp/alps-baselines/dinov2-debug-fair` by default.  `OUTPUT_DIR`,
...
python benchmark_models/debug_running/run_dinov2-small_debug.py \
  --enable-hexkl --enable-alps-items-1-7 --device-iterations 20
```
...
OUTPUT_DIR=/tmp/alps-baselines/dinov2-debug-qnn-fair \
scripts/run_dinov2_qnn_baseline.sh
```
...
| HexKL + Alps items 1-7 warm model body | 1 + DAE scout | no explicit high vote | **51.712 ms** |
...
scope is still 170.7x faster than HVX and 59.6x faster than the Alps
...
5. QNN performs vendor graph preparation: fixed physical layouts, graph-wide
   ...
   runtime advantage.
6. Alps's 51.712 ms result is real, but its logs show zero HexKL rewrites
...
retaining Alps as the data-movement layer.
...
than the current HexKL path, and 52.8x faster than HexKL+Alps items 1-7
...
| B2 | hexagon-mlir plus HexKL/HMX | HMX compute without Alps |
| B3 | hexagon-mlir plus HexKL/HMX plus Alps | proposed movement system |
...
> not part of the current Alps items 1--7 contribution unless a later
...
vs. coverage-expanded HMX plus Alps
```
...
HMX throughput rather than Alps's prefetch, DMA, in-situ reshape, or
...
1. **Current-paper boundary.** Treat improved HMX coverage only as baseline
   ...
   to Alps.  Do not claim padded/batched HMX lowering as an Alps
   contribution.
...
as experimental infrastructure rather than silently folded into Alps.
