...
>
> 本文不是新的功能清单，而是对 OmniFetch 到目前为止的设计、实现、实验和方向修正做一次因果审计。主要依据是
> [创新设计文档](docs/omnifetch-prefetch-insitu-innovation.md)、
...
- OmniFetch 已经形成了完整的编译器机制框架，并证明能够显著减少预取命令数、VTCM 静态峰值和某些布局变换；
...
因此，“所有模型都由同一个 prefetch 开关得到 1.8x”不是可信的科学假设。若所有模型都能达到这个倍率，通常意味着当前 baseline 存在系统性 codegen 缺陷；修复这种缺陷是必要的工程工作，但不能全部算作 OmniFetch 的论文贡献。
...
### 3.1 初始 OmniFetch：从提前搬动到编译器可见的数据供应
...
### 3.4 item 7 与 `omnifetch-2x-improvement`：第一次大幅收益，也是最大归因陷阱
...
即 `2.692x` over HexKL。提交 `b6f5548` 因此被标记为 tag `omnifetch-2x-improvement`。
...
在 `baseline_5` 上实现了 Prefetch-Kernel-HX 和 APT-GET-HX，并与 OmniFetch 在 DINOv2/ViT Debug 上串行对比。OmniFetch 的重要优势是请求数显著更少：

- DINOv2：OmniFetch 36 次 runtime request，对方 122843 次；
- ViT：OmniFetch 72 次，对方 36274 次。
...
- OmniFetch 在两个 Debug 模型上将 prefetch 请求数量降低数个数量级；
...
- tag `omnifetch-2x-improvement` 的 Falcon 2.692x；
...
它是原 OmniFetch、in-situ reshape、M1–M10 和 MAR 的收敛版本，不是新故事。
...
1. 为每个 runner 输出 `scalar / true-HVX / VTCM / HexKL-HMX / OmniFetch` 的实际 option manifest；
...
- HVX、HexKL/HMX、HexKL/HMX+OmniFetch；
...
> 现有 Hexagon 编译路径把数据供应当作各算子的局部问题，导致相邻 operator 之间反复 materialize layout、重复从 L2/DDR 读取、产生互相覆盖的 prefetch，并在 HMX/HVX 边界丢失 residency。OmniFetch 建立跨 producer-consumer 的 movement contract：先消除可避免的 movement，再在产生时形成消费布局，让一次必要 movement 服务多个消费者，最后依据 V73 的 single-flight L2、DMA coherency、VTCM 和 VRF 约束隐藏剩余 latency。
...
1. 保留 `omnifetch-2x-improvement` tag，作为组合策略第一次出现大幅收益的历史锚点；不把它当作当前 true-HVX 论文结果。
...
最重要的是：接下来每一次声称加速，都必须回答三个问题——**究竟少搬了多少物理字节、究竟隐藏了哪一段不可避免的 latency、这一次 movement 究竟服务了几个消费者**。回答不了这三个问题的倍率，不再进入 OmniFetch 的正式结论。
