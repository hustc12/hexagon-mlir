# Alps Design Audit, Model Validation, and Roadmap

Date: 2026-07-26

## Scope

This document records a source-level audit of the Alps implementation in
...
1. unmodified Hexagon-MLIR, with HexKL and Alps disabled; and
...
Alps is a cross-layer compiler/runtime prototype, not a single prefetch
...
dominant speedup. Alps adds approximately 0--4% over HexKL-only on the
...
| B1 | unmodified Hexagon-MLIR, HexKL and Alps off |
...
The full GPT-2 FP16 graph produces NaN logits on HVX, HexKL, and Alps.
Recorded bisection shows FP16 saturation around layers 3--4. This is not an
Alps-only error, but it means the full-model timing is performance evidence
...
The small `verify_alps_model.py` Transformer is not a full language-model
...
| phone GEMM 64x256x512, plain Alps | correct, 8.227 ms |
...
1. **Device-side per-iteration sampling.** `hexagon_benchmark.h` now provides
   ...
   (`hexagon_launcher_base.py`) and the Alps WH-cache warm phase
...
**Aligned-frame fix status.** The earlier local `PS_aligna` workaround is
superseded by three exact official LLVM Hexagon patches under `patches/llvm/`:
truncating subregister COPY handling, AP prologue use-before-def repair, and AP
live-in tracking. They are validated on the previously-faulting vector item-7
Falcon path, DINOv2-debug, and complete DINOv2-small v73 HVX execution. See
`engineering_work.md` sections 11.1 and 17 for commits and full results.
