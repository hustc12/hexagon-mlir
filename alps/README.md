# Alps overlay for Qualcomm Hexagon-MLIR
...
The Alps implementation is maintained as an overlay rather than a forked
...
- Triton and triton-shared are cloned at the revisions selected by Qualcomm's
  ...
  build patch from `alps/patches/triton`;
...
2. Rebase `alps-overlay` onto that snapshot.
3. Resolve only the small integration surface recorded in `manifest.txt`.
4. Run `scripts/script_legacy/verify_alps_overlay.sh` to detect accidental changes to
   Qualcomm-owned files.
5. Run `scripts/script_legacy/build_alps_upstream.sh`.
...
live-in tracking. These are compiler correctness prerequisites, not Alps
performance improvements.

The Hexagon-only dependency patch makes Triton's built-in NVIDIA and AMD
backends optional. Alps sets `TRITON_IN_TREE_BACKENDS` to empty and builds
...
entire compile, test, and 15-model HVX/HexKL/HexKL+Alps matrix is launched
with one command:
...
`/tmp/alps-upstream-full-models` by default. Re-running the command skips
