...
根据 `alps-design-audit-and-roadmap.md` 和代码分析：
...
**问题**: 早期报告的大倍率主要来自基线配置错误，而非 Alps 本身。


### 1.3 完整模型支持不足

当前状态（来自 `alps-prefetch-insitu-innovation.md` 的完整模型执行计划）:
...
**重要**: 这是**基础设施工作**，不是 Alps 贡献

创建独立的 HMX 覆盖率基线:
```
B0: HVX (vectorization=true, VTCM=true)
B1: HexKL/HMX (direct-aligned only, no Alps)
B2: HexKL/HMX (with padding, no Alps) <- P0.1+P0.2 的结果
B3: HexKL/HMX (with padding + Alps items 1-7)
```
...
**解决方案**（来自 `alps-design-audit-and-roadmap.md` § 6）:
```python
...
- [ ] L2 预取运行时符合 V73 规范
- [ ] 所有 Alps 机制与扩展 HMX 兼容

成功标准:
```
典型模型 (如 DINOv2 Debug):
  B2 (HMX padded):           10 ms
  B3 (HMX + Alps):      4-5 ms  (2-2.5x 加速)
...
Attribution:
  ...
  - Alps data supply: ~15% of speedup
...
这样 HMX 扩展是明确的基础设施，Alps 是数据供应层，故事连贯。
...
| B3 (HMX + ALPS) | 60.3 ms (scalar) | **4-5 ms** | P1.1-P1.3 优化 |
...
| B3 (HMX + ALPS) | 11.7s (FP32 问题) | **60-70s** | Mixed precision |
