...
The baselines are deliberately separate from Alps. They may reuse the
same `alps.l2_hint` carrier and the page-safe runtime helper, but they do
not use in-situ reshape, shadow-buffer rewiring, DMA-to-VTCM, V-DAE,
persistent caches, adaptive Alps control, or K/V semantic annotations.

## Existing implementation inventory

The repository already provides:

- `alps.l2_hint`, a destination-free read-only cache-hint operation;
- lowering from a static strided memref region to
  `__alps_l2_hint_2d(base,width,height,stride)`;
...
layout transforms, stage VTCM, and compose with other Alps items.
...
`alps.l2_hint`. It never changes the demand load or consumer buffer.
...
| Row | Address decision | Timing decision | Other Alps mechanisms |
|---|---|---|---|
...
| Alps | Alps analysis | Alps policy | only the explicitly named cumulative row |
...
- Model compilation uses independent options
  ...
  `enablePrefetch` or any other Alps flag.
- The pass emits only `alps.l2_hint`; the existing
  Alps-to-LLVM lowering is enabled solely to translate that carrier to
  `__alps_l2_hint_2d`.
...
- The v73 runtime-test object was disassembled with
  `hexagon-llvm-objdump -d`; `alps_l2fetch_2d` contains the real instruction
...
| Data-movement policy | none; Prefetch-Kernel-HX; APT-GET-HX; Alps | how future data movement/cache hints are selected |

Accordingly, “Hexagon-MLIR” is the no-prefetch framework control, while the
two external methods and Alps are data-movement policies compiled inside
...
output paths use the absolute Android execution directory. Alps allocation
lifetime shortening is now gated behind Alps and cannot contaminate the
external-baseline rows. None of these repairs is an optimization or may be
counted as baseline/Alps speedup.
...
### Alps items 1--7 full-model gate

The preferred next step was evaluated before broadening either external
baseline to all models: run complete Alps items 1--7 on these same two
...
`scripts/run_alps_full_two_models.sh`.

The Alps persistent-cache protocol intentionally invokes the device model
...
| DINOv2-small | Alps items 1--7 | 19,061.970 ms | 171,041,329 cumulative | 287,102,337,728 cumulative | finite; max diff 0.0049; top-1 match |
...
| ViT-Base | Alps items 1--7 | 48,344.123 ms | 398,320,594 cumulative | 669,108,840,110 cumulative | finite; max diff 0.0049; top-1 match |

This full-model gate is negative but diagnostically decisive. Alps is
...
After normalizing the three Alps calls, DINOv2 still issues 10.47x as many
...
traffic storm. The earlier Debug result—where Alps issued far fewer
requests—therefore does not generalize to the full graph under the current
items-1--7 policy.

The validated local result root is
`/home/huzq85/2-working/hexagon_npu/run_artifacts/full_alps_vs_prefetch_20260813`;
the 840-MiB result backup is
`nano:/home/huzq85/2-working/working_set/full_alps_vs_prefetch_20260813`.
...
100% page-clipped. Alps should expand beyond these two models only after
...
Alps optimization and is not one of the seven items. It is the benchmark
...
`scripts/run_alps_full_no_item4_ablation.sh`. Every Alps row uses the
...
19,959.873 ms. That improvement is not an Alps or HMX-kernel claim.
...
`/home/huzq85/2-working/hexagon_npu/run_artifacts/full_alps_no_item4_20260813`
and its generated-artifact backup is
`nano:/home/huzq85/2-working/working_set/full_alps_no_item4_20260813`.
...
therefore added for Alps only: at most 4,096 issued commands and 8 MiB,
...
`/home/huzq85/2-working/hexagon_npu/run_artifacts/full_alps_budget_20260813`
and synchronized to
`nano:/home/huzq85/2-working/working_set/full_alps_budget_20260813`.

### Current default policy: item 7 only

The causal evidence above makes item 7 the current default Alps policy.
...
`--enable-alps-kv-cache-prefetch --disable-layout-aware
--disable-alps-adaptive`, not `--enable-alps-items-1-7`.
...
`--disable-layout-aware --disable-alps-adaptive`. Pass logs confirm
...
| DINOv2-small | Alps item7-only | **5,953.59 ms (1.00x)** |
...
| ViT-Base | Alps item7-only | **14,115.92 ms (1.00x)** |
...
default Alps traffic guard. No additional full-model experiment is needed
...
```bash
OUTPUT_DIR=/tmp/alps-prefetch-baseline5-final2 \
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| DINOv2 Debug | Alps items 1--7 | **18,084** | 18,130 | 19,195 | 16,926 | 4 | 36 | 78,336 / 34,432 | finite, max diff 0.0006, top-1 match |
...
| ViT Debug | Alps items 1--7 | **48,511** | 47,443 | 51,415 | 46,816 | 8 | 72 | 46,080 / 41,408 | finite, max diff 0.0007, top-1 match |
...
Relative to APT-GET-HX and Prefetch-Kernel-HX respectively, the Alps
...
Alps issues orders of magnitude fewer requests: 36 rather than 122,843
...
loops, whereas Alps's semantic/layout-aware policy selects four or eight
...
baseline, rather than only for Alps V-DAE.  This closed a measurement bug
