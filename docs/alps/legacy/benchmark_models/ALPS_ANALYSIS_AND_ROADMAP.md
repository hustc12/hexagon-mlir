# Alps Analysis & Improvement Roadmap

Status: analysis snapshot (branch `alps_2`). This document captures a full read of the
Alps subsystem (dialect/IR, transform passes, LLVM conversion, device runtime),
the relevant Hexagon SDK / HEXKL primitives, and the benchmark models, plus a
prioritized improvement roadmap. It is the working reference for the Alps effort.

---

## 1. Current design blueprint

Alps is a three-component (Plan-A, independently toggled) prefetch subsystem
inserted **after bufferization and before LLVM lowering**, built around HEXKL's HMX
matmul micro-kernels.

### IR layer (`include/hexagon/Dialect/Alps/IR/AlpsOps.td`)
- `alps.prefetch_in_situ` — core op. Args: `src`, `dest` (memrefs),
...
- `alps.create_sem` / `signal` / `wait` — V-DAE semaphores.
- `alps.adaptive_control` — PMU-driven lookahead tuning (see limitation below).
...
5. `AlpsVDAEInsert` (Component 3) — wraps async loops with `create_sem/wait/…/signal`
   (+ optional `adaptive_control`); skips L2Hint and sync-only prefetches.
6. `HexKLToLLVM` + `AlpsToLLVM` — lower to extern-C runtime calls.

### Device runtime (`bin/runtime/src/AlpsRuntime.c`)
...
1. **V-DAE is not true decoupled access-execute.** Runtime is software-pipelined on a
   ...
   (`AlpsRuntime.c:86,381`). SDK offers `worker_pool` (up to 12 workers), an
...
2. **`adaptive_control` is a no-op** — `__alps_update_distance` returns its input
...
7. **Data-credibility gap.** `plotting/ALPS_Prefetcher_Data.csv` shows ALPS 2–4×, but
   `PHASE4_STATUS_AND_ALPS.md` measures Alps at **0–4% over HexKL**, and the CSV
...
- **Baseline A = Hexagon-MLIR (vanilla HVX):** this repo with HexKL/Alps off
...
the dominant win (GPT-2 ~2×, tiny LLM 10–20×) while Alps is a small increment.
...
| Issue | Impact |
|---|---|
| **GPT-2 12L device logits all NaN** (all 3 configs) | Only full-depth LLM with complete on-device A/B; results invalid. Likely fp16 saturation (~after 4 layers). Blocks Alps numeric validation. |
