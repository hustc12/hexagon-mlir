...
| 项 | 内容 | 状态 | 位置 |
|----|------|------|------|
| #2 | adaptive_control 真实化 | **已实现(未提交)** | `bin/runtime/src/AlpsRuntime.c` |
...
- 信号量：`volatile int alps_sem_pool[16]`（单 HW 线程，volatile 足够）。
- HexKL 权重 WH 字节格式是**闭源**的；`libsdkl` 只有 ARM/DSP，无 x86。任何 WH 预打包
  **必须在设备上跑真实 `hexkl_micro_hmx_rm_to_wh_f16`**，禁止在 x86 复刻格式。
- 组件门控：`enablePrefetch` 是 #2/#3(部分)/VDAE 的基座；`AlpsToLLVM` 在
  `enablePrefetch || enableAlpsWeightPrepack` 时运行(`LinalgToLLVMPass.cpp:491-492`)。
...
**已落地代码**（`bin/runtime/src/AlpsRuntime.c`）：
- 参数：`MIN_LOOKAHEAD 1` / `MAX_LOOKAHEAD 8` / `STALL_THRESHOLD 8000`（line 45-50）。
- 全局：`alps_stall_accum`(自旋累计)、`alps_stall_events`(wait 次数)、
  `alps_eff_lookahead`(当前有效预取距离，初值 MAX)（line 59-61）。
- `__alps_wait`：局部 `spins` 累加进 `alps_stall_accum` / `alps_stall_events++`（line 411-412）。
- `__alps_update_distance`：读并清零累计，算均摊 `avg=accum/events`；
  `avg>STALL_THRESHOLD` → 距离+1（上限 MAX）；`avg*4<STALL_THRESHOLD` → -1（下限 MIN）；
  写回 `alps_eff_lookahead` 并返回（line 711-739）。
- async 预取深度消费 `alps_eff_lookahead`：预热接下来 (alps_eff_lookahead-1) 个 K-tile
  （line 303-305）。

**接线点(未改动，已存在)**：`VDAEDecouplePass.cpp:197-209` 生成 `AdaptiveControlOp`；
`AlpsToLLVMPass.cpp` LowerAdaptiveControl → `__alps_update_distance(i32)->i32`。

**待验证**：`enableAlpsAdaptive=True` 编 GPT-2，设备结果与关闭 adaptive 一致（自适应只影响
时序不影响数值），延迟不劣化。
...
- 内层 K-loop：用 `alps.prefetch_in_situ(whTile→vtcm+curW, LayoutTransform::None,
  lookahead=0)` 把预打包 tile 拷进 VTCM 槽（line 310-338），保留 ping-pong `curW` 让 Alps
  仍可重叠。
- `whW` 在 outer M-loop 后 dealloc（line 382-383）。option 关闭时走原路径不变。

**接线**：`hexagon_options.py enableAlpsWeightPrepack=False`(默认关)；
`Passes.td` 的 `DecomposeHexKLMatmul` option `enable-weight-prepack`；
`LinalgToLLVMPass.cpp:400 decomposeOptions.enableWeightPrepack=enableAlpsWeightPrepack`；
...
**现状**：async DMA 在 `__alps_signal → alps_async_complete`(AlpsRuntime.c:120-126,
...
- 仅在 `enableAlpsVDAE=True` 且循环有 async prefetch(lookahead>0) 时启用。
...
ninja 会因 `AlpsRuntime.c` 变更重新生成 `.bc` → 字节数组（#2/#3 runtime 改动被吸收），
C++ pass 走主 ninja。
...
4. 已知：Alps 相对 HexKL 实测仅 ~0–4%；HexKL 才是主要收益(2–24×)。见 `plan_todo.md` Results。
...
- Alps 降级：`qcom_hexagon_backend/lib/Conversion/AlpsToLLVM/AlpsToLLVMPass.cpp`
- 外部函数名：`qcom_hexagon_backend/include/hexagon/Conversion/AlpsToLLVM/AlpsExternalFnNames.h`
- Op 定义：`qcom_hexagon_backend/include/hexagon/Dialect/Alps/IR/AlpsOps.td`
- runtime：`qcom_hexagon_backend/bin/runtime/src/AlpsRuntime.c`、`VTCMPool.cpp`、`UserDMA/`
