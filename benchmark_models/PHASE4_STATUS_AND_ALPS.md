# Phase-4 Status, Harness Topology & Alps Assessment
...
They only wrapped three sequential debug runs (HVX → HexKL → HexKL+Alps).
...
| Mode | Topology | Device 3-way (HVX / HexKL / HexKL+ALPS) |
|------|----------|--------------------------------------|
...
## 5. Alps: prefetch + in-situ reshape — did it show up?
...
Layout-aware Alps on the HexKL weight path:
...
| Vehicle | HexKL | HexKL+ALPS | Δ vs HexKL | Inserts / notes |
|---------|-------|----------|------------|-----------------|
...
| Qwen earlier ALPS run | 155.8 ms | 153.0 ms | ~1–2% | ~30 sites (15× layout-fusion) |

**Caveat (full GPT-2):** device `Result:Pass` and Perf timings are usable for ALPS vs HexKL ranking, but Hexagon logits are **NaN** on all three configs (HVX included); debug 2L was Top-1 OK. Treat as perf/ablation evidence pending NaN root-cause.

### Where inserts happened but time was ~parity (noise / no clear win)

| Vehicle | Notes |
|---------|--------|
| GPT-2 2L fair | ALPS ~11111 ms vs HexKL 11110 ms; 8×2 async inserts |
...
3. **ALPS vs HexKL** stays in the **~0–4%** band even on full GPT-2 (~**3.1%**) — larger depth/width **did not** unlock a double-digit ALPS story; still proves path is live at published topology.
4. Likely reasons ALPS stays muted:
...
5. **Best ALPS demos:** GEMM 256³ (~3%), Qwen/Falcon tiny (~3–4%), **GPT-2 full 12L (~3.1%)** — consistent small win class.

### What would make ALPS more visible next
...
# AlpsRuntime.c / DMA / device bitcode → must rebuild hexagon_runtime
...
| Change | Rebuild |
|--------|---------|
| `AlpsRuntime.c` / DMA / scout | `hexagon_runtime` (+ `hexagon_mlir_async_runtime` if scout enqueue) |
...
| `--enable-alps-vdae` | `enablePrefetch` + `enableAlpsVDAE` | Alps + V-DAE |
| `--enable-alps-weight-prepack` | `enableAlpsWeightPrepack` | VTCM-resident WH prepack (column-outer) |
| `--enable-alps-dual-thread-dae` | `enableAlpsDualThreadDae` | Scout thread for deferred WH (needs VDAE) |
| `--enable-alps-inter-layer-prefetch` | `enableAlpsInterLayerPrefetch` | Outer HexKL loop prefetch |
| `--enable-alps-attention-hmx` | `enableAlpsAttentionHmx` | Pad K==M/N==M attn matmuls → HexKL |
| `--enable-alps-dma-to-vtcm` | `enableAlpsDmaToVtcm` | DMA stage in VTCM (default **off**; DDR stage) |
| `--enable-hexkl-persistent-vtcm` | `enableHexKLPersistentVtcm` | Shared VTCM arena (default off) |
| `--alps-lookahead N` | `alpsLookahead` | Lookahead (default 2) |
| `--disable-layout-aware` | layout-aware off | Linear prefetch only |
| `--disable-alps-adaptive` | adaptive off | Stall adaptive off |

### 7.2 Phase-1 style A/B (HexKL / ALPS / prepack)
...
python $DBG --enable-hexkl --seq-len 32 --enable-alps-vdae
python $DBG --enable-hexkl --seq-len 32 --enable-alps-weight-prepack

# seq=128 (prepack win scales with ceil(M/32))
python $DBG --enable-hexkl --seq-len 128
python $DBG --enable-hexkl --seq-len 128 --enable-alps-vdae
python $DBG --enable-hexkl --seq-len 128 --enable-alps-weight-prepack
```
...
python $DBG --enable-hexkl --seq-len 32 \
  --enable-alps-vdae --enable-alps-dual-thread-dae

# Inter-layer prefetch
python $DBG --enable-hexkl --seq-len 32 \
  --enable-alps-vdae --enable-alps-inter-layer-prefetch

# Attention in-flight pad → HexKL
python $DBG --enable-hexkl --seq-len 32 --enable-alps-attention-hmx
```

### 7.4 Micro GEMM smoke

```bash
python benchmark_models/verify_alps_Gemm.py --enable-hexkl --m 64 --k 128 --n 256
python benchmark_models/verify_alps_Gemm.py --enable-hexkl --enable-alps --enable-layout-aware
python benchmark_models/verify_alps_Gemm.py --enable-hexkl --enable-alps-weight-prepack --m 64 --k 128 --n 256
```
...
python benchmark_models/run_gpt2lmheadmodel.py --seq-len 32 --enable-hexkl --enable-alps-vdae
```
...
| HexKL+ALPS | 345.7 s | 1337.2 s |
| HexKL+prepack | 334.5 s | **1278.2 s (~4% vs HexKL)** |
| ALPS+dual-thread | 316.4 s | — |
| ALPS+inter-layer | 322.6 s | — |
