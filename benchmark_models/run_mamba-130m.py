...
def mamba_130m(enablelwp=False): 

    model_name = "state-spaces/mamba-130m"
    prompt = "What is nature of our existence?"
    
    # Mamba official repository specifies using EleutherAI/gpt-neox-20b for tokenization
    tokenizer = AutoTokenizer.from_pretrained("EleutherAI/gpt-neox-20b")

    config = AutoConfig.from_pretrained(model_name)
    # Using fewer layers for a "Lite" compilation test
    config.num_hidden_layers = 2 
    config.use_cache = False

    # Note: Using from_pretrained will download the weights directly from Hugging Face.
    # We use from_config here to avoid state_dict size mismatch errors (config has vocab_size 50277, 
    # but the checkpoint has 50280 due to padding). Random weights are sufficient for compiler testing.
    model = AutoModelForCausalLM.from_config(config, torch_dtype=torch.float16)
    class MambaWrapper(torch.nn.Module):
        def __init__(self, m):
            super().__init__()
            self.embed = m.backbone.embeddings
    ...
    module = compile_to_linalg(wrapped_model, (input_ids,))
    ...
    # Compare directly instead of using get_top_5 since output is not logits
    diff = (hex_outputs[0] - x86_outputs).abs().max()
    print(f"Max diff: {diff}")
    if diff < 0.1:
        print("Hexagon and CPU results match")
    else:
        print("Hexagon and CPU results do not match")
    if enablelwp:
        process_lwp()
