# ===- verify_alps_Gemm.py ----------------------------------------------===
#
# Phase-0 / Phase-1 vehicle for Alps on a real HexKL MicroHMX path.
...
def compile_and_run(
    ...
    if verbose:
        ...
        print(f"  HexKL={enable_hexkl} Alps={enable_alps} "
    ...
    options = HexagonOptions(
        ...
        enableAlpsWeightPrepack=enable_weight_prepack,
    ...
    mlir_path = "/tmp/verify_alps_gemm.mlirbc"
...
def run_phase0(m=256, k=256, n=256):
    configs = [
        ("HexKL only",
         dict(enable_alps=False, enable_hexkl=True, enable_layout_aware=False,
              lookahead=0, adaptive=False)),
        ("HexKL + Alps (layout OFF)",
         dict(enable_alps=True, enable_hexkl=True, enable_layout_aware=False,
              lookahead=2, adaptive=True)),
        ("HexKL + Alps (layout ON)",
         dict(enable_alps=True, enable_hexkl=True, enable_layout_aware=True,
              lookahead=2, adaptive=True)),
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--enable-alps", action="store_true")
    ...
    if args.phase0:
        ...
        compile_and_run(
            ...
            enable_weight_prepack=args.enable_alps_weight_prepack,
