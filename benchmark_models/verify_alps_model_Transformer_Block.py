...
class AlpsVerifModel(nn.Module):
    """
    Minimal model: a single TransformerBlock for ALPS verification.
...
def compile_and_run(
    enable_alps: bool = False,
    enable_hexkl: bool = False,
    lookahead: int = 2,
    adaptive: bool = False
):
    print(f"\n{'='*60}")
    print(f"  ALPS Verification Model")
    ...
    device = "cpu"
    model = AlpsVerifModel().to(device).half()
    model.eval()
    
    # Input: [1, seq_len, dim] matching AlpsVerifModel defaults
    ...
    options["enableAlpsVDAE"] = enable_alps
    options["enableAlpsLayoutAware"] = True
    options["alpsLookahead"] = lookahead
    options["enableAlpsAdaptive"] = adaptive
    ...
    options["enableVTCMTiling"] = enable_alps
    # Optional: Enable VTCM Tiling if helpful for prefetching (only with Alps)
    # options["enableVTCMTiling"] = enable_alps
    options["htp_kernel_gen"] = True

    # print(f'enableVectorization = {enable_hexkl}')
    # print(f'enableHexKL = {enable_hexkl}')
    # print(f'enableAlpsVDAE = {enable_alps}')
    print(f'{HexagonOptions().__dict__}')
