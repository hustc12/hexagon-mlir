//===- AlpsRuntime.c - ALPS device-side runtime ----------------===//
//
// Copyright (c) Qualcomm Technologies, Inc. and/or its subsidiaries.
// SPDX-License-Identifier: BSD-3-Clause.
//
// Hexagon device-side runtime for the ALPS prefetching system.
...
/* -------------------------------------------------------------------------
 * Layout kind constants – must match AlpsOps.td enum ordinals
 * ------------------------------------------------------------------------- */
...
#define ALPS_ERROR_SEM_TIMEOUT (1u << 0)
#define ALPS_ERROR_DESCRIPTOR_FULL (1u << 1)
#define ALPS_ERROR_DESCRIPTOR_STALE (1u << 2)
#define ALPS_ERROR_DESCRIPTOR_STATE (1u << 3)
#define ALPS_ERROR_CONTEXT_BUSY (1u << 4)
static unsigned alps_error_flags = 0;

uint32_t __alps_get_and_clear_errors(void) {
  return __atomic_exchange_n(&alps_error_flags, 0, __ATOMIC_ACQ_REL);
}
...
static void alps_descriptor_fail(unsigned error) {
  ...
  __atomic_fetch_or(&alps_error_flags, error, __ATOMIC_RELAXED);
}
...
int32_t __alps_invocation_begin(void) {
  ...
  alps_descriptor_fail(ALPS_ERROR_CONTEXT_BUSY);
  return -1;
}

int32_t __alps_invocation_end(int32_t context_handle) {
  unsigned slot;
  uint32_t generation;
  if (!alps_decode_context(context_handle, &slot, &generation)) {
    alps_descriptor_fail(ALPS_ERROR_DESCRIPTOR_STALE);
    return 0;
  }
  AlpsInvocationContext *context = &alps_contexts[slot];
  for (unsigned i = 0; i < ALPS_DESCRIPTORS_PER_CONTEXT; ++i) {
...
int32_t __alps_descriptor_acquire(int32_t context_handle,
  ...
  alps_descriptor_fail(ALPS_ERROR_DESCRIPTOR_FULL);
  return -1;
}

int32_t __alps_descriptor_transition(int32_t descriptor_handle,
...
int32_t __alps_descriptor_consume(int32_t descriptor_handle,
                                        int64_t value_version, int64_t tile,
                                        int32_t layout, int32_t source_tier,
                                        int32_t destination_tier) {
  AlpsInvocationContext *context;
...
int32_t __alps_descriptor_release(int32_t descriptor_handle) {
  AlpsInvocationContext *context;
  AlpsExactDescriptor *descriptor;
  if (!alps_decode_descriptor(descriptor_handle, &context, &descriptor)) {
    (void)context;
...
typedef struct {
  ...
  int32_t site_id;
  volatile int valid;
  unsigned char data[ALPS_WH_TILE_BYTES] __attribute__((aligned(128)));
} AlpsWhCacheEntry;
...
static unsigned alps_wh_hits = 0;
static unsigned alps_wh_misses = 0;
 ...
 * unobservable hint. */
static unsigned alps_l2_issued = 0;
static unsigned alps_l2_busy_suppressed = 0;
static unsigned alps_l2_page_clipped = 0;
static unsigned alps_l2_unsupported = 0;
static uint64_t alps_l2_requested_bytes = 0;
static uint64_t alps_l2_issued_bytes = 0;
static unsigned alps_l2_budget_suppressed = 0;
static unsigned alps_l2_duplicate_suppressed = 0;
static unsigned alps_l2_max_commands = 0;
static uint64_t alps_l2_max_bytes = 0;
#define ALPS_L2_RECENT_REQUESTS 64
#define ALPS_L2_SEGMENTED_SITES 256
static uint64_t alps_l2_recent_requests[ALPS_L2_RECENT_REQUESTS];
static unsigned alps_l2_recent_count = 0;
static unsigned alps_l2_recent_cursor = 0;
static unsigned alps_l2_segmented_cursor[ALPS_L2_SEGMENTED_SITES];

/* Configure a per-invocation traffic envelope for Alps. Zero limits keep
 * the legacy/unbounded behavior used by external prefetch baselines. */
void __alps_l2_configure(uint32_t max_commands, uint64_t max_bytes,
                               uint32_t recent_requests) {
  alps_l2_max_commands = max_commands;
  alps_l2_max_bytes = max_bytes;
  alps_l2_recent_count = recent_requests < ALPS_L2_RECENT_REQUESTS
                             ? recent_requests
                             : ALPS_L2_RECENT_REQUESTS;
  alps_l2_recent_cursor = 0;
  for (unsigned i = 0; i < ALPS_L2_RECENT_REQUESTS; ++i)
    alps_l2_recent_requests[i] = 0;
  for (unsigned i = 0; i < ALPS_L2_SEGMENTED_SITES; ++i)
    alps_l2_segmented_cursor[i] = 0;
  __atomic_store_n(&alps_l2_issued, 0, __ATOMIC_RELAXED);
  __atomic_store_n(&alps_l2_busy_suppressed, 0, __ATOMIC_RELAXED);
  __atomic_store_n(&alps_l2_page_clipped, 0, __ATOMIC_RELAXED);
  __atomic_store_n(&alps_l2_unsupported, 0, __ATOMIC_RELAXED);
  __atomic_store_n(&alps_l2_requested_bytes, 0, __ATOMIC_RELAXED);
  __atomic_store_n(&alps_l2_issued_bytes, 0, __ATOMIC_RELAXED);
  __atomic_store_n(&alps_l2_budget_suppressed, 0, __ATOMIC_RELAXED);
  __atomic_store_n(&alps_l2_duplicate_suppressed, 0, __ATOMIC_RELAXED);
}

uint64_t __alps_l2_scheduler_counts(void) {
  uint64_t issued = __atomic_load_n(&alps_l2_issued, __ATOMIC_RELAXED);
  uint64_t busy = __atomic_load_n(&alps_l2_busy_suppressed, __ATOMIC_RELAXED);
  return (issued << 32) | (busy & UINT64_C(0xffffffff));
}

uint64_t __alps_l2_scheduler_limits(void) {
  uint64_t clipped = __atomic_load_n(&alps_l2_page_clipped, __ATOMIC_RELAXED);
  uint64_t unsupported =
      __atomic_load_n(&alps_l2_unsupported, __ATOMIC_RELAXED);
  return (clipped << 32) | (unsupported & UINT64_C(0xffffffff));
}

uint64_t __alps_l2_requested_bytes(void) {
  return __atomic_load_n(&alps_l2_requested_bytes, __ATOMIC_RELAXED);
}

uint64_t __alps_l2_issued_bytes(void) {
  return __atomic_load_n(&alps_l2_issued_bytes, __ATOMIC_RELAXED);
}

uint64_t __alps_l2_policy_suppressed(void) {
  uint64_t budget =
      __atomic_load_n(&alps_l2_budget_suppressed, __ATOMIC_RELAXED);
  uint64_t duplicate =
      __atomic_load_n(&alps_l2_duplicate_suppressed, __ATOMIC_RELAXED);
...
#define ALPS_W8_CACHE_SLOTS 512
#define ALPS_W8_TILE_ELEMS 1024
#define ALPS_W8_GROUP_ROWS 8
#define ALPS_W8_GROUPS (32 / ALPS_W8_GROUP_ROWS)
typedef struct {
  uint64_t context;
  uint32_t generation;
  uint32_t epoch;
  ...
  float scales[ALPS_W8_GROUPS * 32];
  volatile int valid;
  int8_t data[ALPS_W8_TILE_ELEMS] __attribute__((aligned(128)));
} AlpsW8CacheEntry;

static AlpsW8CacheEntry alps_w8_cache[ALPS_W8_CACHE_SLOTS];
static unsigned alps_w8_hits = 0;
static unsigned alps_w8_misses = 0;
static void hmx_weight_gather(const void *src, void *dest, int32_t elem_bytes,
                              int32_t M, int32_t K);

uint64_t __alps_w8_cache_stats(void) {
  uint64_t hits = __atomic_load_n(&alps_w8_hits, __ATOMIC_RELAXED);
  uint64_t misses = __atomic_load_n(&alps_w8_misses, __ATOMIC_RELAXED);
  return (hits << 32) | (misses & 0xffffffffu);
}

static unsigned alps_w8_hash(const void *source, int32_t tile_row,
  ...
  x ^= x >> 33;
  return (unsigned)x & (ALPS_W8_CACHE_SLOTS - 1);
}

static AlpsW8CacheEntry *alps_w8_lookup_or_quantize(const _Float16 *source,
                                                    int32_t tile_row,
                                                    int32_t tile_col,
                                                    int32_t source_cols,
                                                    int32_t site_id) {
  ...
  unsigned slot = alps_w8_hash(source, tile_row, tile_col, source_cols, site_id,
                               context, generation);
  AlpsW8CacheEntry *entry = &alps_w8_cache[slot];
  if (__atomic_load_n(&entry->valid, __ATOMIC_ACQUIRE) &&
    ...
    __atomic_fetch_add(&alps_w8_hits, 1, __ATOMIC_RELAXED);
    return entry;
  }
  ...
  const _Float16 *row0 = source + (size_t)tile_row * 32 * (size_t)source_cols +
                         (size_t)tile_col * 32;
  for (int group = 0; group < ALPS_W8_GROUPS; ++group)
    for (int c = 0; c < 32; ++c) {
      float max_abs = 0.0f;
      int row_begin = group * ALPS_W8_GROUP_ROWS;
      for (int rr = 0; rr < ALPS_W8_GROUP_ROWS; ++rr) {
        float value =
  ...
  __atomic_fetch_add(&alps_w8_misses, 1, __ATOMIC_RELAXED);
  return entry;
}

static void alps_w8_dequant_to_wh(const _Float16 *source, void *dest,
                                  int32_t weight_off, int32_t tile_row,
                                  int32_t tile_col, int32_t source_cols,
                                  int32_t site_id) {
  AlpsW8CacheEntry *entry = alps_w8_lookup_or_quantize(
      source, tile_row, tile_col, source_cols, site_id);
  _Float16 dequant[ALPS_W8_TILE_ELEMS] __attribute__((aligned(128)));
  for (int r = 0; r < 32; ++r)
    for (int c = 0; c < 32; ++c) {
      float scale = entry->scales[(r / ALPS_W8_GROUP_ROWS) * 32 + c];
...
static unsigned alps_wh_cache_hash(const void *source, int32_t tile_row,
                                   int32_t tile_col, int32_t source_cols,
                                   int32_t site_id, uint64_t context,
                                   uint32_t generation) {
  ...
  x ^= x >> 33;
  return (unsigned)x & (ALPS_WH_CACHE_SLOTS - 1);
}

void __alps_wh_cache_set_context(uint64_t context, uint32_t generation) {
  uint64_t old_context = __atomic_load_n(&alps_wh_context, __ATOMIC_ACQUIRE);
  uint32_t old_generation =
      __atomic_load_n(&alps_wh_generation, __ATOMIC_ACQUIRE);
  if (old_context != context || old_generation != generation)
    __atomic_add_fetch(&alps_wh_epoch, 1, __ATOMIC_ACQ_REL);
  __atomic_store_n(&alps_wh_context, context, __ATOMIC_RELEASE);
  __atomic_store_n(&alps_wh_generation, generation, __ATOMIC_RELEASE);
}

void __alps_wh_cache_invalidate(uint64_t context, uint32_t generation) {
  __atomic_add_fetch(&alps_wh_epoch, 1, __ATOMIC_ACQ_REL);
  for (int i = 0; i < ALPS_WH_CACHE_SLOTS; ++i) {
    if (__atomic_load_n(&alps_wh_cache[i].valid, __ATOMIC_ACQUIRE) &&
        alps_wh_cache[i].context == context &&
...
uint64_t __alps_wh_cache_stats(void) {
  uint64_t hits = __atomic_load_n(&alps_wh_hits, __ATOMIC_RELAXED);
  uint64_t misses = __atomic_load_n(&alps_wh_misses, __ATOMIC_RELAXED);
  return (hits << 32) | (misses & 0xffffffffu);
}

static AlpsWhCacheEntry *
alps_wh_cache_lookup(const void *source, int32_t tile_row, int32_t tile_col,
  ...
  for (unsigned i = 0; i < ALPS_WH_CACHE_PROBES; ++i) {
    AlpsWhCacheEntry *entry =
        &alps_wh_cache[(base + i) & (ALPS_WH_CACHE_SLOTS - 1)];
    if (__atomic_load_n(&entry->valid, __ATOMIC_ACQUIRE) &&
        entry->context == context && entry->generation == generation &&
...
static AlpsWhCacheEntry *
alps_wh_cache_reserve(const void *source, int32_t tile_row, int32_t tile_col,
                      int32_t source_cols, int32_t site_id) {
  uint64_t context = __atomic_load_n(&alps_wh_context, __ATOMIC_ACQUIRE);
  uint32_t generation = __atomic_load_n(&alps_wh_generation, __ATOMIC_ACQUIRE);
  uint32_t epoch = __atomic_load_n(&alps_wh_epoch, __ATOMIC_ACQUIRE);
  unsigned base = alps_wh_cache_hash(source, tile_row, tile_col, source_cols,
                                     site_id, context, generation);
  AlpsWhCacheEntry *entry = &alps_wh_cache[base];
  for (unsigned i = 0; i < ALPS_WH_CACHE_PROBES; ++i) {
    AlpsWhCacheEntry *candidate =
        &alps_wh_cache[(base + i) & (ALPS_WH_CACHE_SLOTS - 1)];
 ...
 *   alps_stall_accum  – sum of spin-wait iterations observed in wait()
 *   alps_stall_events – number of wait() calls contributing to the sum
 *   alps_eff_lookahead– current effective prefetch distance in [MIN,MAX];
 *                       adjusted by __alps_update_distance from the
 *                       measured average stall and consumed by the prefetch
 *                       path (async-gate + L2 prefetch-ahead depth). */
static unsigned alps_stall_accum = 0;
static int alps_stall_events = 0;
static int alps_eff_lookahead = MAX_LOOKAHEAD;
...
static volatile int alps_dual_thread_dae = 0;

enum { ALPS_SLOT_IDLE = 0, ALPS_SLOT_PENDING = 1, ALPS_SLOT_READY = 2 };
static volatile int alps_scout_slot_state = ALPS_SLOT_IDLE;

/* Strong symbol from multithreading/AlpsScout.cpp overrides this weak
...
void __alps_set_dual_thread_dae(int32_t enable) {
  __atomic_store_n(&alps_dual_thread_dae, enable ? 1 : 0, __ATOMIC_RELEASE);
  __atomic_store_n(&alps_scout_slot_state, ALPS_SLOT_IDLE, __ATOMIC_RELEASE);
}
...
#define ALPS_SEM_MAX_SPIN 0x100000

typedef int alps_sem_t;
static alps_sem_t alps_sem_pool[ALPS_SEM_POOL_SIZE];
static unsigned alps_sem_generation[ALPS_SEM_POOL_SIZE];
static int alps_sem_alloc_idx = 0;
...
static int alps_async_count = 0;

/* Defined with the async-DMA section below; signal() finishes deferred WH. */
static void alps_async_complete(void);

static void alps_sem_pool_init(void) {
  static int initialised = 0;
  int expected = 0;
  if (__atomic_compare_exchange_n(&initialised, &expected, 1, 0,
                                  __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE)) {
    for (int i = 0; i < ALPS_SEM_POOL_SIZE; ++i)
      __atomic_store_n(&alps_sem_pool[i], 0, __ATOMIC_RELAXED);
    for (int i = 0; i < ALPS_SEM_POOL_SIZE; ++i)
      __atomic_store_n(&alps_sem_generation[i], 0, __ATOMIC_RELAXED);
...
static void alps_scout_complete_and_ready(void *sem_idx_as_ptr) {
  __atomic_store_n(&alps_scout_slot_state, ALPS_SLOT_PENDING, __ATOMIC_RELEASE);
  alps_async_complete();
  __atomic_store_n(&alps_scout_slot_state, ALPS_SLOT_READY, __ATOMIC_RELEASE);
  int32_t sem_handle = (int32_t)(intptr_t)sem_idx_as_ptr;
  unsigned sem_idx = (unsigned)sem_handle & (ALPS_SEM_POOL_SIZE - 1);
  unsigned generation = (unsigned)sem_handle / ALPS_SEM_POOL_SIZE;
  if (sem_idx < ALPS_SEM_POOL_SIZE &&
      __atomic_load_n(&alps_sem_generation[sem_idx], __ATOMIC_ACQUIRE) ==
          generation)
    __atomic_fetch_add(&alps_sem_pool[sem_idx], 1, __ATOMIC_RELEASE);
}

int32_t __alps_create_sem(void) {
  alps_sem_pool_init();
  int32_t idx = __atomic_fetch_add(&alps_sem_alloc_idx, 1, __ATOMIC_RELAXED) %
                ALPS_SEM_POOL_SIZE;
  unsigned generation =
      __atomic_add_fetch(&alps_sem_generation[idx], 1, __ATOMIC_ACQ_REL);
  __atomic_store_n(&alps_sem_pool[idx], 0, __ATOMIC_RELEASE);
  return (int32_t)(generation * ALPS_SEM_POOL_SIZE + (unsigned)idx);
}

void __alps_signal(int32_t sem_handle) {
  unsigned sem_idx = (unsigned)sem_handle & (ALPS_SEM_POOL_SIZE - 1);
  unsigned generation = (unsigned)sem_handle / ALPS_SEM_POOL_SIZE;
  if (sem_idx >= ALPS_SEM_POOL_SIZE ||
      __atomic_load_n(&alps_sem_generation[sem_idx], __ATOMIC_ACQUIRE) !=
 ...
 * DDR tile into a staging buffer and return immediately.  __alps_wait
...
void alps_hmx_async_drain_start_f16(void *vtcm, int32_t in_offset,
  ...
  uint32_t token = hexagon_runtime_dma2d_start(
      src, ALPS_VTCM, out, ALPS_DDR, columns * 2u, 32u,
...
#define ALPS_STAGE_ELEMS (32 * 32)
#define ALPS_STAGE_SLOTS 4

static uint16_t alps_stage[ALPS_STAGE_SLOTS][ALPS_STAGE_ELEMS];
...
enum {
  ALPS_JOB_IDLE = 0,
  ALPS_JOB_LOAD_PENDING = 1,
  ALPS_JOB_LOAD_READY = 2,
  ALPS_JOB_TRANSFORM_READY = 3
};
...
static AlpsAsyncJob alps_async_jobs[ALPS_STAGE_SLOTS];
static int alps_async_head = 0;
static int alps_async_tail = 0;
static int alps_async_consumer_lock = 0;
...
static int32_t alps_exact_find(int32_t context_handle, int64_t value_version,
  ...
  for (unsigned slot = 0; slot < ALPS_DESCRIPTORS_PER_CONTEXT; ++slot) {
    ...
    if (state == ALPS_DESC_FREE ||
        ...
        descriptor->source_tier != ALPS_DDR ||
        descriptor->destination_tier != ALPS_VTCM)
      continue;
    uint32_t generation =
...
#else
  uint16_t stage[ALPS_STAGE_ELEMS];
...
int32_t __alps_exact_weight_kick(int32_t context_handle,
  ...
  int32_t descriptor_handle =
      __alps_descriptor_acquire(context_handle, value_version, tile,
                                      LAYOUT_HMX_WEIGHT, ALPS_DDR, ALPS_VTCM);
  AlpsInvocationContext *context;
  AlpsExactDescriptor *descriptor;
...
int32_t __alps_exact_weight_consume(int32_t context_handle,
                                          int64_t value_version,
                                          int32_t tile_row, int32_t tile_col) {
  int32_t descriptor_handle =
  ...
  return __alps_descriptor_consume(
      ...
      alps_pack_tile_identity(tile_row, tile_col), LAYOUT_HMX_WEIGHT, ALPS_DDR,
      ALPS_VTCM);
}

int32_t __alps_exact_weight_release(int32_t context_handle,
                                          int64_t value_version,
                                          int32_t tile_row, int32_t tile_col) {
  int32_t descriptor_handle =
      alps_exact_find(context_handle, value_version, tile_row, tile_col);
  if (descriptor_handle < 0)
    return 0;
  return __alps_descriptor_release(descriptor_handle);
}
...
uint64_t __alps_exact_overlap_counts(void) {
  uint64_t scout =
...
uint64_t __alps_exact_consume_waits(void) {
...
uint64_t __alps_p4a_issue_cycles(void) {
  return __atomic_load_n(&alps_p4a_issue_cycles, __ATOMIC_RELAXED);
}

uint64_t __alps_p4a_poll_cycles(void) {
  return __atomic_load_n(&alps_p4a_poll_cycles, __ATOMIC_RELAXED);
}

uint64_t __alps_p4a_poll_retries(void) {
  return __atomic_load_n(&alps_p4a_total_poll_retries, __ATOMIC_RELAXED);
}

uint64_t __alps_p4a_pmu_values01(void) {
  return ((uint64_t)alps_p4a_pmu_delta[0] << 32) |
         (uint64_t)alps_p4a_pmu_delta[1];
}

uint64_t __alps_p4a_pmu_values23(void) {
  return ((uint64_t)alps_p4a_pmu_delta[2] << 32) |
         (uint64_t)alps_p4a_pmu_delta[3];
}

static void alps_async_complete(void) {
  if (__atomic_load_n(&alps_async_count, __ATOMIC_ACQUIRE) <= 0)
    return;
  while (__atomic_exchange_n(&alps_async_consumer_lock, 1, __ATOMIC_ACQUIRE)) {
#ifdef __hexagon__
    __asm__ volatile("pause(#64)");
#endif
  }
  if (__atomic_load_n(&alps_async_count, __ATOMIC_ACQUIRE) <= 0) {
    __atomic_store_n(&alps_async_consumer_lock, 0, __ATOMIC_RELEASE);
    return;
  }
  int job_idx = __atomic_load_n(&alps_async_head, __ATOMIC_RELAXED);
  AlpsAsyncJob *job = &alps_async_jobs[job_idx];
...
#endif
  __atomic_store_n(&job->phase, ALPS_JOB_LOAD_READY, __ATOMIC_RELEASE);
  void *dest = job->dest;
  ...
  if (job->hexkl_deferred) {
    ...
    if (dest && job->weight_off >= 0) {
      ...
      if (job->vtcm_stage_off >= 0) {
        ...
        whSrc = (const _Float16 *)alps_stage[job->stage_slot];
      }
      if (whSrc) {
      ...
      if (job->site_id >= 0) {
        AlpsWhCacheEntry *entry =
            alps_wh_cache_reserve(job->src, job->tile_row, job->tile_col,
                                  job->src_cols, job->site_id);
        memcpy(entry->data, (char *)dest + job->weight_off, ALPS_WH_TILE_BYTES);
        if (entry->epoch == __atomic_load_n(&alps_wh_epoch, __ATOMIC_ACQUIRE))
          __atomic_store_n(&entry->valid, 1, __ATOMIC_RELEASE);
      }
    }
    ...
    (void)ne;
#endif
    __atomic_store_n(&job->phase, ALPS_JOB_TRANSFORM_READY, __ATOMIC_RELEASE);
    __atomic_store_n(&job->active, 0, __ATOMIC_RELEASE);
    __atomic_store_n(&alps_async_head, (job_idx + 1) % ALPS_STAGE_SLOTS,
                     __ATOMIC_RELAXED);
    __atomic_fetch_sub(&alps_async_count, 1, __ATOMIC_RELEASE);
    __atomic_store_n(&alps_async_consumer_lock, 0, __ATOMIC_RELEASE);
    return;
  }

  const void *staged = alps_stage[job->stage_slot];
...
#ifdef __hexagon__
static void alps_l2fetch_2d(const void *ptr, uint32_t width, uint32_t height,
                            uint32_t stride);

static void alps_l2fetch_weight_tile(const _Float16 *src, int32_t tile_row,
                                     int32_t tile_col, int32_t src_cols) {
  const _Float16 *row0 =
      src + (size_t)tile_row * 32 * (size_t)src_cols + (size_t)tile_col * 32;
  ...
  alps_l2fetch_2d(row0, /*width=*/32u * 2u, /*height=*/32u,
                  /*stride=*/(uint32_t)src_cols * 2u);
}
#endif

/* Phase 2b HexKL kick: dma2d-pack the next strided weight tile into VTCM
 * staging (flatOff) when stage_off>=0, else DDR alps_stage.  Returns so Mm
 * overlaps the transfer; __alps_signal drains DMA and WHs into the idle
 * ping-pong weight slot (slab+weight_off). */
static int alps_async_kick_hexkl_weight(const void *src, void *dest,
                                        int32_t tile_row, int32_t tile_col,
  ...
  if (__atomic_load_n(&alps_async_count, __ATOMIC_ACQUIRE) >=
      ALPS_STAGE_SLOTS) {
    int spins = 0;
    while (__atomic_load_n(&alps_async_count, __ATOMIC_ACQUIRE) >=
               ALPS_STAGE_SLOTS &&
           spins < ALPS_SEM_MAX_SPIN) {
      ++spins;
#ifdef __hexagon__
    ...
    if (__atomic_load_n(&alps_async_count, __ATOMIC_ACQUIRE) >=
        ALPS_STAGE_SLOTS) {
      alps_async_complete();
      __atomic_fetch_or(&alps_error_flags, ALPS_ERROR_DESCRIPTOR_FULL,
                        __ATOMIC_RELAXED);
    }
  }

  int job_idx = __atomic_load_n(&alps_async_tail, __ATOMIC_RELAXED);
  AlpsAsyncJob *job = &alps_async_jobs[job_idx];
...
#ifdef __hexagon__
  alps_l2fetch_weight_tile((const _Float16 *)src, tile_row, tile_col, src_cols);
  /* Do NOT l2fetch tile_row+ah unconditionally: alps_eff_lookahead can be up to
  ...
  if (stage_off >= 0) {
    ...
    tok = hexagon_runtime_dma2d_start(
        (void *)row0, ALPS_DDR, vtcmDst, ALPS_VTCM, width, height, srcStride,
    ...
    slot = job_idx;
    tok = hexagon_runtime_dma2d_start(
        (void *)row0, ALPS_DDR, alps_stage[slot], ALPS_DDR, width, height,
        srcStride, dstStride, /*bypassSrc=*/0, /*bypassDst=*/0, /*isOrdered=*/0,
        /*cacheAllocationPolicy=*/0, &status);
    if (status != ALPS_DMA_OK) {
      alps_pack_weight_tile_to_stage((const _Float16 *)src, tile_row, tile_col,
                                     src_cols, alps_stage[slot]);
      tok = 0;
    }
  }
#else
  ...
  job->src = src;
  job->elem_bytes = 2;
  job->num_elems = ALPS_STAGE_ELEMS;
  ...
  job->site_id = site_id;
  __atomic_store_n(&job->phase, ALPS_JOB_LOAD_PENDING, __ATOMIC_RELEASE);
  __atomic_store_n(&job->active, 1, __ATOMIC_RELEASE);
  __atomic_store_n(&alps_async_tail, (job_idx + 1) % ALPS_STAGE_SLOTS,
                   __ATOMIC_RELAXED);
  __atomic_fetch_add(&alps_async_count, 1, __ATOMIC_RELEASE);
  return 1;
}

static int alps_async_kick(const void *src, void *dest, int32_t elem_bytes,
                           int32_t num_elems, int32_t layout_kind) {
  uint32_t bytes = (uint32_t)elem_bytes * (uint32_t)num_elems;
  if (bytes == 0 || bytes > sizeof(alps_stage[0]) || !src || !dest)
    return 0;
  if (__atomic_load_n(&alps_async_count, __ATOMIC_ACQUIRE) >= ALPS_STAGE_SLOTS)
    alps_async_complete();
  if (__atomic_load_n(&alps_async_count, __ATOMIC_ACQUIRE) >= ALPS_STAGE_SLOTS)
    return 0;
  ...
  uint32_t tok = hexagon_runtime_dma_start(
      (void *)src, ALPS_DDR, alps_stage[slot], ALPS_DDR, bytes,
      /*bypassSrc=*/0, /*bypassDst=*/0, &status);
  if (status != ALPS_DMA_OK) {
    memcpy(alps_stage[slot], src, bytes);
    tok = 0;
  }
  job->token = tok;
#else
...
void __alps_wait(int32_t sem_handle) {
  ...
  if (sem_idx >= ALPS_SEM_POOL_SIZE ||
    ...
    return;
  int spins = 0;
  while (__atomic_load_n(&alps_sem_pool[sem_idx], __ATOMIC_ACQUIRE) <= 0) {
    if (__atomic_load_n(&alps_sem_generation[sem_idx], __ATOMIC_ACQUIRE) !=
        generation)
      return;
    if (++spins >= ALPS_SEM_MAX_SPIN) {
      __atomic_fetch_or(&alps_error_flags, ALPS_ERROR_SEM_TIMEOUT,
                        __ATOMIC_RELAXED);
      break;
    }
#ifdef __hexagon__
  ...
  __atomic_fetch_add(&alps_stall_accum, (unsigned)spins, __ATOMIC_RELAXED);
  __atomic_fetch_add(&alps_stall_events, 1, __ATOMIC_RELAXED);
  int observed = __atomic_load_n(&alps_sem_pool[sem_idx], __ATOMIC_ACQUIRE);
  while (observed > 0 && !__atomic_compare_exchange_n(
                             &alps_sem_pool[sem_idx], &observed, observed - 1,
                             0, __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE)) {
  }
}
...
#ifdef __hexagon__
enum {
  ALPS_L2_PAGE_BYTES = 4096,
  ALPS_L2_RECOMMENDED_MAX_BYTES = 8191,
...
static unsigned alps_l2fetch_active(void) {
  uint32_t usr;
  __asm__ volatile("%0 = usr" : "=r"(usr));
  return (usr >> ALPS_USR_PFA_BIT) & 1u;
}

static void alps_l2fetch_2d(const void *ptr, uint32_t width, uint32_t height,
                            uint32_t stride) {
  uint64_t requested = (uint64_t)width * (uint64_t)height;
  __atomic_fetch_add(&alps_l2_requested_bytes, requested, __ATOMIC_RELAXED);
  if (!ptr || width == 0 || height == 0 || stride == 0 || width > UINT16_MAX ||
      height > UINT16_MAX || stride > UINT16_MAX) {
    __atomic_fetch_add(&alps_l2_unsupported, 1, __ATOMIC_RELAXED);
    return;
  }

  uintptr_t start = (uintptr_t)ptr;
  uintptr_t startPage = start & ~(uintptr_t)(ALPS_L2_PAGE_BYTES - 1);
  uint32_t pageRoom = ALPS_L2_PAGE_BYTES - (uint32_t)(start - startPage);
  uint32_t safeWidth = width < pageRoom ? width : pageRoom;
  uint32_t maxRowsByBytes = ALPS_L2_RECOMMENDED_MAX_BYTES / safeWidth;
  ...
  while (safeHeight > 1) {
    ...
    if ((lastStart & ~(uintptr_t)(ALPS_L2_PAGE_BYTES - 1)) == startPage &&
        (lastEnd & ~(uintptr_t)(ALPS_L2_PAGE_BYTES - 1)) == startPage)
      break;
    --safeHeight;
  }
  if (safeWidth != width || safeHeight != height)
    __atomic_fetch_add(&alps_l2_page_clipped, 1, __ATOMIC_RELAXED);
  ...
  unsigned issued = __atomic_load_n(&alps_l2_issued, __ATOMIC_RELAXED);
  uint64_t issuedBytes =
      __atomic_load_n(&alps_l2_issued_bytes, __ATOMIC_RELAXED);
  uint64_t remainingBytes =
      issuedBytes < alps_l2_max_bytes ? alps_l2_max_bytes - issuedBytes : 0;
  if ((alps_l2_max_commands && issued >= alps_l2_max_commands) ||
      (alps_l2_max_bytes && safeBytes > remainingBytes)) {
    __atomic_fetch_add(&alps_l2_budget_suppressed, 1, __ATOMIC_RELAXED);
    return;
  }
  ...
  if (alps_l2_recent_count) {
    ...
    if (requestKey == 0)
      requestKey = 1;
    for (unsigned i = 0; i < alps_l2_recent_count; ++i) {
      if (alps_l2_recent_requests[i] == requestKey) {
        __atomic_fetch_add(&alps_l2_duplicate_suppressed, 1, __ATOMIC_RELAXED);
        return;
      }
    }
    alps_l2_recent_requests[alps_l2_recent_cursor] = requestKey;
    alps_l2_recent_cursor = (alps_l2_recent_cursor + 1) % alps_l2_recent_count;
  }

  /* Do not overwrite a useful V73 command.  This is deliberately a
   * nonblocking suppression rather than a compute-thread spin. */
  if (alps_l2fetch_active()) {
    __atomic_fetch_add(&alps_l2_busy_suppressed, 1, __ATOMIC_RELAXED);
    return;
  }
  ...
  __atomic_fetch_add(&alps_l2_issued, 1, __ATOMIC_RELAXED);
  __atomic_fetch_add(&alps_l2_issued_bytes,
...
static void alps_l2fetch(const void *ptr, uint32_t total_bytes) {
  ...
  if (width > ALPS_L2_RECOMMENDED_MAX_BYTES)
    width = ALPS_L2_RECOMMENDED_MAX_BYTES;
  alps_l2fetch_2d(ptr, width, /*height=*/1, /*stride=*/width);
}
#endif

void __alps_l2_hint_2d(const void *src, int32_t width_bytes,
                             int32_t height, int32_t stride_bytes) {
  if (!src || width_bytes <= 0 || height <= 0 || stride_bytes <= 0)
    return;
#ifdef __hexagon__
  alps_l2fetch_2d(src, (uint32_t)width_bytes, (uint32_t)height,
...
void __alps_l2_hint_segmented(const void *src, int32_t width_bytes,
  ...
  unsigned slot = (unsigned)site_id % ALPS_L2_SEGMENTED_SITES;
  unsigned cursor =
      __atomic_fetch_add(&alps_l2_segmented_cursor[slot], 1, __ATOMIC_RELAXED);
  unsigned row = cursor % (unsigned)rows;
  const char *row_ptr = (const char *)src + (size_t)row * stride_bytes;
  alps_l2fetch_2d(row_ptr, (uint32_t)width_bytes, /*height=*/1,
 ...
 * ------------------------------------------------------------------------- */
void __alps_prefetch_insitu(const void *src, void *dest,
  ...
  if (layout_kind == LAYOUT_HMX_ACTIVATION && src_cols > 0 && tile_row >= 0 &&
     ...
     * "Alps enabled" ablation with zero issued requests. */
    alps_l2fetch_weight_tile((const _Float16 *)src, tile_row, tile_col,
                             src_cols);
  ...
  if (layout_kind == LAYOUT_HMX_WEIGHT_DEQUANT_I8 && src_cols > 0 &&
    ...
    alps_w8_dequant_to_wh((const _Float16 *)src, dest, weight_off, tile_row,
  ...
  if (layout_kind == LAYOUT_HMX_WEIGHT && src_cols > 0 && tile_row >= 0 &&
    ...
    if (lookahead == -1) {
      AlpsWhCacheEntry *cached =
          alps_wh_cache_lookup(src, tile_row, tile_col, src_cols, src_rows);
      if (cached) {
        memcpy(wh_dest, cached->data, ALPS_WH_TILE_BYTES);
        __atomic_fetch_add(&alps_wh_hits, 1, __ATOMIC_RELAXED);
        return;
      }
      __atomic_fetch_add(&alps_wh_misses, 1, __ATOMIC_RELAXED);
    }
    if (lookahead > 0 && weight_off >= 0) {
      ...
      if (alps_async_kick_hexkl_weight(src, dest, tile_row, tile_col, src_cols,
    ...
    if (lookahead == -1) {
      AlpsWhCacheEntry *entry =
          alps_wh_cache_reserve(src, tile_row, tile_col, src_cols, src_rows);
      memcpy(entry->data, wh_dest, ALPS_WH_TILE_BYTES);
      if (entry->epoch == __atomic_load_n(&alps_wh_epoch, __ATOMIC_ACQUIRE))
        __atomic_store_n(&entry->valid, 1, __ATOMIC_RELEASE);
    }
    return;
  ...
  if (lookahead > 0 &&
    ...
    if (alps_async_kick(src, dest, elem_bytes, num_elems, layout_kind))
      return;
...
/* -------------------------------------------------------------------------
 * __alps_update_distance  (adaptive lookahead control)
 * ------------------------------------------------------------------------- */
int32_t __alps_update_distance(int32_t current_dist) {
   ...
   * __alps_wait since the previous update as the timeliness signal. */
  int events = __atomic_exchange_n(&alps_stall_events, 0, __ATOMIC_ACQ_REL);
  unsigned accum = __atomic_exchange_n(&alps_stall_accum, 0, __ATOMIC_ACQ_REL);

  int dist = (current_dist >= MIN_LOOKAHEAD && current_dist <= MAX_LOOKAHEAD)
                 ? current_dist
                 : __atomic_load_n(&alps_eff_lookahead, __ATOMIC_ACQUIRE);
  ...
  __atomic_store_n(&alps_eff_lookahead, dist, __ATOMIC_RELEASE);
  return dist;
}
