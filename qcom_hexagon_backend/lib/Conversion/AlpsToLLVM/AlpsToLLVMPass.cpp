//===- AlpsToLLVMPass.cpp - alps → LLVM lowering  --------------===//
//
...
//===----------------------------------------------------------------------===//
//
// Lowers all alps dialect ops to extern-C calls into the Hexagon device
// runtime (AlpsRuntime.c), keeping the compiler and runtime decoupled.
//
...
#include "hexagon/Conversion/AlpsToLLVM/AlpsExternalFnNames.h"
...
#define DEBUG_TYPE "alps-to-llvm"
...
#define GEN_PASS_DEF_ALPSTOLLVM
...
/// Get or insert `i32 __alps_create_sem()`.
...
/// Get or insert `void __alps_signal(i32)`.
static FailureOr<LLVM::LLVMFuncOp>
...
/// Get or insert `void __alps_wait(i32)`.
static FailureOr<LLVM::LLVMFuncOp>
...
/// Get or insert the `__alps_prefetch_insitu` declaration.
static FailureOr<LLVM::LLVMFuncOp>
...
/// Get or insert `void __alps_l2_hint_2d(ptr, i32, i32, i32)`.
static FailureOr<LLVM::LLVMFuncOp>
...
/// Get or insert `void __alps_l2_hint_segmented(ptr, i32, i32, i32,
/// i32)`.
static FailureOr<LLVM::LLVMFuncOp>
...
/// Get or insert `void __alps_copy2d(...)`.
static FailureOr<LLVM::LLVMFuncOp>
...
/// Get or insert `i32 __alps_update_distance(i32)`.
static FailureOr<LLVM::LLVMFuncOp>
...
//===----------------------------------------------------------------------===//
// CreateSemOp  →  i32 __alps_create_sem()
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// SignalOp  →  void __alps_signal(i32 sem)
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// WaitOp  →  void __alps_wait(i32 sem)
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// PrefetchInSituOp  →  void __alps_prefetch_insitu(…)
//                 or   void __alps_copy2d(…) for rank-2 LAYOUT_NONE
//
...
//===----------------------------------------------------------------------===//
// L2HintOp → read-only __alps_l2_hint_2d(...)
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// AdaptiveControlOp  →  i32 __alps_update_distance(i32)
//===----------------------------------------------------------------------===//
...
// Pass definition
//===----------------------------------------------------------------------===//
struct AlpsToLLVMPass
    : public ::impl::AlpsToLLVMBase<AlpsToLLVMPass> {

  using AlpsToLLVMBase::AlpsToLLVMBase;
  ...
  void runOnOperation() override {
    ...
    target.addIllegalDialect<AlpsDialect>();
...
// Public factory
//===----------------------------------------------------------------------===//
std::unique_ptr<Pass> mlir::alps::createAlpsToLLVMPass(
    const AlpsToLLVMOptions &options) {
  return std::make_unique<AlpsToLLVMPass>(options);
}
