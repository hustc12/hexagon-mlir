//===- AlpsDialect.cpp - Alps Dialect Implementation  -----------===//
//
...
void AlpsDialect::initialize() {
  registerOperations();

  addAttributes<
#define GET_ATTRDEF_LIST
#include "hexagon/Dialect/Alps/IR/AlpsAttrs.cpp.inc"
      >();
}

Attribute AlpsDialect::parseAttribute(DialectAsmParser &parser,
                                           Type type) const {
  StringRef tag;
  if (failed(parser.parseKeyword(&tag)))
    return {};
  if (tag != "layout_transform") {
    parser.emitError(parser.getNameLoc(), "unknown alps attribute: ")
        << tag;
    return {};
  }
...
void AlpsDialect::printAttribute(Attribute attr,
...
//===----------------------------------------------------------------------===//
#include "hexagon/Dialect/Alps/IR/AlpsDialect.cpp.inc"
