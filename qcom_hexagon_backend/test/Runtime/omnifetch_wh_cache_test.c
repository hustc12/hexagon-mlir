...
int main(void) {
  ...
  __omni_fetch_wh_cache_set_context(7, 1);
  __omni_fetch_prefetch_insitu(weight, first, 2, 1024, 1, -1, 0, 0, 0,
                               32, 0, -1, -1);
  __omni_fetch_prefetch_insitu(weight, second, 2, 1024, 1, -1, 0, 0, 0,
                               32, 0, -1, -1);
  uint64_t stats = __omni_fetch_wh_cache_stats();
  if ((uint32_t)(stats >> 32) != 1 || (uint32_t)stats != 1 ||
      memcmp(first, second, sizeof(first)) != 0)
    return 1;

  __omni_fetch_wh_cache_invalidate(7, 1);
  __omni_fetch_prefetch_insitu(weight, second, 2, 1024, 1, -1, 0, 0, 0,
                               32, 0, -1, -1);
  stats = __omni_fetch_wh_cache_stats();
  if ((uint32_t)(stats >> 32) != 1 || (uint32_t)stats != 2)
    return 2;

  puts("OmniFetch WH cache contract: PASS");
  return 0;
}
