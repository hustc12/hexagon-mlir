...
for model in "${models[@]}"; do
  ...
  "${repo_root}/scripts/prepare_phone_benchmark.sh" apply >"${output}/phone_before.txt"
  echo "START model=${model} $(date --iso-8601=seconds)"
  set +e
  HEXAGON_MLIR_COMPILE_THREADS=4 \
  HEXAGON_MLIR_DUMP_DIR="${output}/artifacts" \
    python "${repo_root}/scripts/probe_full_audio_hotspots.py" "${model}" \
      >"${output}/run.log" 2>&1
  rc=$?
  set -e
  if ((rc)); then
  ...
  python "${repo_root}/scripts/summarize_alps_lwp.py" \
  ...
  "${repo_root}/scripts/prepare_phone_benchmark.sh" status >"${output}/phone_after.txt"
  printf 'PASS\n' >"${output}/status.txt"
