...
#   bash scripts/build_hexagon_mlir_incremental.sh --full
#   bash scripts/build_hexagon_mlir_incremental.sh --tests
#   bash scripts/build_hexagon_mlir_incremental.sh --full --clean --tests
#
set -euo pipefail
...
CLEAN_BUILD=0
FULL_BUILD=0
HEXAGON_ARCH="${HEXAGON_ARCH_VERSION:-75}"
...
export LLVM_PROJECT_BUILD_DIR="${LLVM_PROJECT_BUILD_DIR:-${PARENT_DIR}/LLVM_DIR/llvm-project/build}"
export CONDA_ENV="${CONDA_ENV:-${PARENT_DIR}/mlir-env}"

require_dir() {
  [[ -d "$1" ]] || {
    echo "Required directory is missing: $1" >&2
    echo "Provision dependencies with scripts/build_hexagon_mlir.sh or follow docs/user-guide.md." >&2
    exit 1
  }
}
...
if [[ "${FULL_BUILD}" -eq 0 && -f "${BUILD_DIR}/build.ninja" ]]; then
  ...
  cmake --build "${BUILD_DIR}" --parallel "${BUILD_JOBS}" \
    --target linalg-hexagon-opt hexagon_runtime libtriton.so
else
