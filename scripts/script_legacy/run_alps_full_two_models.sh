#!/usr/bin/env bash
# Run full DINOv2-small and ViT-Base with the current Alps item-7-only
# policy, then join
...
output_dir=${OUTPUT_DIR:-${parent_dir}/run_artifacts/full_alps_vs_prefetch_$(date +%Y%m%d_%H%M%S)}
...
record_result() {
  ...
  if [[ -z "${perf}" || -z "${correctness}" ]]; then
    echo "Invalid Alps result in ${log}: perf=${perf:-missing} correctness=${correctness:-missing}" >&2
    return 1
  fi
  printf '%s,alps-item7-only,pass,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "${model}" "${perf}" "${p50}" "${p90}" "${min}" "${sites}" \
    "${issued}" "${busy}" "${clipped}" "${requested}" "${issued_bytes}" \
    "${correctness}" >> "${output_dir}/alps_results.csv"
}

run_model() {
  local model=$1 runner=$2
  local model_dir="${output_dir}/${model}"
  local log="${model_dir}/alps-item7-only.log"
  ...
  if [[ "${reuse_valid_logs}" == 1 && -f "${log}" ]] && \
    ...
    echo "[SerialRun] reuse-valid model=${model} scheme=alps-item7-only"
    record_result "${model}" "${log}"
    sync_remote
    return 0
  fi
  echo "[SerialRun] model=${model} scheme=alps-item7-only"
...
"${venv}/bin/python" - "${baseline_csv}" "${output_dir}/alps_results.csv" "${output_dir}/comparison.csv" <<'PY'
import csv
import sys

baseline_path, alps_path, output_path = sys.argv[1:]
rows = []
...
with open(alps_path, newline="", encoding="utf-8") as handle:
    for row in csv.DictReader(handle):
...
for model in {row["model"] for row in rows}:
    alps = next(float(row["perf_us"]) for row in rows
                if row["model"] == model and row["scheme"] == "alps-item7-only")
    for row in rows:
        if row["model"] == model:
            scheme_latency = float(row["perf_us"])
            # Values greater than one unambiguously mean that Alps is
            # slower than the row's scheme.  Keep the reciprocal ratio too so
            # downstream plotting never has to guess the numerator.
            row["alps_slowdown_vs_scheme"] = f'{alps / scheme_latency:.6f}'
            row["scheme_latency_over_alps"] = f'{scheme_latency / alps:.6f}'

fields = ["model", "scheme", "perf_us", "alps_slowdown_vs_scheme",
          "scheme_latency_over_alps", "issued", "requested_bytes",
          "issued_bytes", "correctness"]
