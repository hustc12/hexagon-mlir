#!/usr/bin/env bash
# Full-model causal ablation for OmniFetch with item 4 disabled.
...
output_dir=${OUTPUT_DIR:-${parent_dir}/run_artifacts/full_omnifetch_no_item4_$(date +%Y%m%d_%H%M%S)}
...
run_model() {
  ...
  run_case "${model}" "${runner}" items1-3 \
    --enable-hexkl --omnifetch-items-through 3
  run_case "${model}" "${runner}" items1-5-no4 \
    --enable-hexkl --omnifetch-items-through 5 \
    --disable-omnifetch-persistent-wh-cache
  run_case "${model}" "${runner}" items1-6-no4 \
    --enable-hexkl --omnifetch-items-through 6 \
    --disable-omnifetch-persistent-wh-cache
  run_case "${model}" "${runner}" items1-7-no4 \
    --enable-hexkl --omnifetch-items-through 7 \
    --disable-omnifetch-persistent-wh-cache
}
