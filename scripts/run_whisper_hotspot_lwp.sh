...
"${repo_root}/scripts/prepare_phone_benchmark.sh" apply >"${local_root}/phone_before.txt"
set +e
HEXAGON_MLIR_COMPILE_THREADS=4 HEXAGON_MLIR_DUMP_DIR="${local_root}/artifacts" \
  python "${repo_root}/scripts/probe_whisper_hotspots.py" >"${local_root}/run.log" 2>&1
rc=$?; set -e
if ((rc)); then printf 'FAIL_%s\n' "$rc" >"${local_root}/status.txt"; exit "$rc"; fi
python "${repo_root}/scripts/summarize_alps_lwp.py" --artifact-root "${local_root}/artifacts" \
...
"${repo_root}/scripts/prepare_phone_benchmark.sh" status >"${local_root}/phone_after.txt"
