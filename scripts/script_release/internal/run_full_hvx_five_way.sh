...
usage() {
  cat <<EOF
Usage: scripts/script_release/internal/run_full_hvx_five_way.sh [options] [model ...]
...
Options:
  ...
  --profile-lwp-loop-depth N
                          LWP loop depth used by profile modes (default: 1)
  ...
  --with-item7            Compose item7 attention/KV topology with P5m or P5n
  ...
  --alps-two-way        Run only HexKL-On and ALPS-C+full-E+P+R
...
runner_for() {
  case "$1" in
    gpt2) echo scripts/script_release/internal/probe_gpt2_layered_export.py ;;
    sd-clip) echo scripts/script_release/internal/probe_clip_layered_export.py ;;
    qwen2.5-0.5b|tinyllama-1.1b|smollm2-1.7b)
      echo scripts/script_release/internal/probe_qwen_layered_export.py
      ;;
...
while (($#)); do
  case "$1" in
    ...
    --alps-p1-hexkl-profile)
      ...
      ;;
    --profile-lwp-loop-depth)
      [[ $# -ge 2 ]] || { echo "--profile-lwp-loop-depth needs a value" >&2; exit 2; }
      profile_lwp_loop_depth=$2; shift 2
      ;;
    ...
    --alps-p5n)
      ...
      ;;
    --with-item7) with_item7=1; shift ;;
    --item7-only) alps_p0=1; item7_only_run=1; shift ;;
    ...
    --alps-p4a)
      ...
      ;;
    --alps-fp16-hvx) alps_p0=1; alps_fp16_hvx=1; shift ;;
    --alps-hvx-widening-conv) alps_p0=1; alps_hvx_widening_conv=1; shift ;;
    --alps-c-e)
      alps_p0=1; alps_p2e=1; alps_hvx_widening_conv=1; shift
      ;;
    --alps-c-e-p)
      alps_p0=1; alps_p2e=1; alps_hvx_widening_conv=1; alps_c_e_p=1; shift
      ;;
    --alps-c-e-p-r)
    ...
    --alps-full-matrix)
      ...
      ;;
    --alps-two-way)
      alps_full_matrix=1; alps_two_way=1; shift
      ;;
...
elif ((alps_hvx_widening_conv && alps_p2e)); then
  schemes=(alps-hvx-widening-conv alps-hvx-widening-conv-consumer-layout)
elif ((alps_hvx_widening_conv)); then
  schemes=(hmlir-hvx-hexkl-on alps-hvx-widening-conv)
elif ((alps_fp16_hvx)); then
  schemes=(hmlir-hvx-hexkl-on alps-fp16-hvx-arithmetic)
elif ((item7_only_run)); then
  schemes=(item7-only)
elif ((alps_p5n)); then
...
if ((with_item7)); then
  ((alps_p5m || alps_p5n)) || {
    echo "--with-item7 is currently supported only with --alps-p5m/--alps-p5n" >&2
    exit 2
  }
  schemes=("${schemes[0]}-item7")
fi

[[ "${iterations}" =~ ^[1-9][0-9]*$ ]] || { echo "Invalid iterations" >&2; exit 2; }
[[ "${seq_len}" =~ ^[1-9][0-9]*$ ]] || { echo "Invalid sequence length" >&2; exit 2; }
[[ "${profile_lwp_loop_depth}" =~ ^[0-9]+$ ]] || { echo "Invalid LWP loop depth" >&2; exit 2; }
...
move_case_to_remote() {
  local model=$1 scheme=$2 case_dir=$3
  local replay=${ALPS_ENABLE_MODEL_SYSMON_REPLAY:-0} preserve_summary=0
  if ((alps_p1_profile)) || [[ "${replay,,}" == 1 || "${replay,,}" == true || "${replay,,}" == yes ]]; then
    preserve_summary=1
  fi
  ...
  if ((alps_p0 && !preserve_summary)); then
    [[ -n "${output_dir}" && "${case_dir}" == "${output_dir}/"* ]] || {
...
audit_case_codegen() {
  ...
  for object_dir in "${object_dirs[@]}"; do
    "${repo_root}/scripts/script_release/internal/audit_hexagon_codegen.sh" "${object_dir}" "${csv}" \
...
extract_alps_p1_ledger() {
  ...
  "${venv}/bin/python" "${repo_root}/scripts/script_release/internal/summarize_alps_movement_ledger.py" \
...
collect_alps_p1_profile() {
  ...
  "${venv}/bin/python" "${repo_root}/scripts/script_release/internal/summarize_alps_lwp.py" \
...
collect_sysmon_profile() {
  local case_dir=$1
  find "${case_dir}/artifacts" -type f -name kernel_window_summary.json \
    -print -quit 2>/dev/null | grep -q . || return 0
  "${venv}/bin/python" "${repo_root}/scripts/script_release/internal/summarize_model_sysmon.py" \
    --artifact-root "${case_dir}/artifacts" \
    --json "${case_dir}/sysmon_model_summary.json" \
    --csv "${case_dir}/sysmon_stages.csv" \
    --markdown "${case_dir}/sysmon_model_summary.md"
}

collect_model_sysmon_replay() {
  local case_dir=$1 enabled=${ALPS_ENABLE_MODEL_SYSMON_REPLAY:-0}
  [[ "${enabled,,}" == 1 || "${enabled,,}" == true || "${enabled,,}" == yes ]] \
    || return 0
  "${repo_root}/scripts/script_release/internal/profile_archived_hexagon_model.sh" \
...
base_args_for() {
  ...
  if ((alps_p1_profile)); then
    printf '%s\n' --enable-lwp --lwp-loop-depth "${profile_lwp_loop_depth}"
  fi
...
scheme_args_for() {
  local scheme=$1 candidate_ids=${2:-} include_item7=0
  if [[ "${scheme}" == *-item7 ]]; then
    include_item7=1
    scheme=${scheme%-item7}
  fi
  case "${scheme}" in
...
upsert_result() {
  ...
  # Layered Language models emit one async-drain telemetry record per stage.
  # The generic extraction above intentionally records the last L2 scheduler
  # state, which made a synchronously-falling-back vocabulary head overwrite
  # the 24 valid transformer-layer DMA counts with zero.  For P5n, aggregate
  ...
  if [[ "${scheme%-item7}" == alps-hmx-async-drain ||
    ...
    issued_bytes=$(awk '/^ALPSHMXAsyncDrain:/{
      for(i=1;i<=NF;i++)if(index($i,"issued_bytes=")==1){v=$i;sub("issued_bytes=","",v);s+=v}
    }END{print s+0}' "${log}")
  fi
  ...
  recorded_log=${log}
  if ((alps_p0 || alps_full_matrix)); then
...
run_case() {
  ...
  local result_scheme execution_scheme full_e=0
  local -a args=()
  ...
  if [[ ${execution_scheme} == alps-hmx-async-drain ]]; then
    scheme=alps-hmx-async-drain-analysis
  fi
  # The frozen E component is the complete consumer-driven representation
  # formation stack, not only the generic P2e matcher.  The maintained item7
  # flag supplies the proven attention propagation/tiling/slicing topology;
  # P2g supplies continuity/register-tile machinery, while P5h and P5i are
  # topology-specific realizers for attention destinations and patch-token
  # producers.  Keep each internal gate independently ablatable, but make every
  # formal C+E/P/R endpoint use the same full E definition.
  ...
  set +e
    ...
    ALPS_ENABLE_REGISTER_TILE_FORMATION="$([[ ${full_e} -eq 1 || ${scheme} == alps-register-tile-direct || ${scheme} == alps-crp-supply-analysis || ${scheme} == alps-crp-supply-prefetch || ${scheme} == alps-crp-segmented-supply || ${scheme} == alps-crp-vtcm-formation || ${scheme} == alps-crp-vtcm-window || ${scheme} == alps-crp-vtcm-async-window || ${scheme} == alps-crp-producer-direct-analysis || ${scheme} == alps-crp-producer-direct-vtcm || ${scheme} == alps-crp-producer-direct-head-major || ${scheme} == alps-crp-producer-head-major-contiguous || ${scheme} == alps-attention-destination-formation || ${scheme} == alps-patch-conv-formation || ${scheme} == alps-hmx-f16-epilogue-formation || ${scheme} == alps-hmx-direct-output-formation || ${scheme} == alps-hmx-f16-bias-epilogue-formation || ${scheme} == alps-hmx-async-drain-analysis ]] && echo 1 || echo 0)" \
    ...
    ALPS_ENABLE_CRP_SUPPLY_ANALYSIS="$([[ ${full_e} -eq 1 || ${scheme} == alps-crp-supply-analysis || ${scheme} == alps-crp-supply-prefetch || ${scheme} == alps-crp-segmented-supply || ${scheme} == alps-crp-vtcm-formation || ${scheme} == alps-crp-vtcm-window || ${scheme} == alps-crp-vtcm-async-window || ${scheme} == alps-crp-producer-direct-analysis || ${scheme} == alps-crp-producer-direct-vtcm || ${scheme} == alps-crp-producer-direct-head-major || ${scheme} == alps-crp-producer-head-major-contiguous || ${scheme} == alps-attention-destination-formation || ${scheme} == alps-patch-conv-formation || ${scheme} == alps-hmx-f16-epilogue-formation || ${scheme} == alps-hmx-direct-output-formation || ${scheme} == alps-hmx-f16-bias-epilogue-formation || ${scheme} == alps-hmx-async-drain-analysis ]] && echo 1 || echo 0)" \
    ...
    ALPS_ENABLE_CRP_PRODUCER_DIRECT_HEAD_MAJOR="$([[ ${full_e} -eq 1 || ${scheme} == alps-crp-producer-direct-head-major || ${scheme} == alps-crp-producer-head-major-contiguous || ${scheme} == alps-attention-destination-formation || ${scheme} == alps-patch-conv-formation || ${scheme} == alps-hmx-f16-epilogue-formation || ${scheme} == alps-hmx-direct-output-formation || ${scheme} == alps-hmx-f16-bias-epilogue-formation || ${scheme} == alps-hmx-async-drain-analysis ]] && echo 1 || echo 0)" \
  ...
  if ((rc == 0)) && grep -q '^[[:space:]]*Perf:' "${log}"; then
    ...
    collect_sysmon_profile "${case_dir}" || true
  ...
  collect_model_sysmon_replay "${case_dir}"
  collect_sysmon_profile "${case_dir}"
  audit_case_codegen "${case_dir}"
  move_case_to_remote "${model}" "${scheme}" "${case_dir}"
...
with_item7 = bool(int(sys.argv[38]))
item7_only_run = bool(int(sys.argv[39]))
alps_fp16_hvx = bool(int(sys.argv[40]))
...
def selected_reference_base():
    ...
    if alps_c_e_p:
        return "alps-c-e-hmx-async-drain"
    if alps_hvx_widening_conv and alps_p2e:
        return "alps-hvx-widening-conv-consumer-layout"
    if alps_hvx_widening_conv: return "alps-hvx-widening-conv"
    if alps_fp16_hvx: return "alps-fp16-hvx-arithmetic"
...
def selected_reference():
    if item7_only_run:
        return "item7-only"
    base = selected_reference_base()
    return f"{base}-item7" if with_item7 else base
...
if alps_fp16_hvx:
    title = "ALPS FP16 HVX arithmetic ablation"
if alps_hvx_widening_conv:
    title = "ALPS C helper-free HVX widening convolution"
if alps_hvx_widening_conv and alps_p2e:
    title = "ALPS C + E consumer-driven direct layout"
if alps_c_e_p:
...
schemes = (
    ...
    if alps_c_e_p else
    ("alps-hvx-widening-conv", "alps-hvx-widening-conv-consumer-layout")
    if alps_hvx_widening_conv and alps_p2e else
    ("hmlir-hvx-hexkl-on", "alps-hvx-widening-conv")
    if alps_hvx_widening_conv else
    ("hmlir-hvx-hexkl-on", "alps-fp16-hvx-arithmetic")
    if alps_fp16_hvx else
...
if with_item7:
    schemes = tuple(f"{scheme}-item7" for scheme in schemes)
    title += " + item7 topology"
if item7_only_run:
    schemes = ("item7-only",)
    title = "Item7-only matched rerun"
...
PY
  if ((alps_two_way)); then
    "${venv}/bin/python" "${repo_root}/scripts/script_release/internal/summarize_alps_two_way.py" \
      --output-root "${output_dir}" \
      --results "${results}" \
      --csv "${output_dir}/alps_full_e_two_way.csv" \
      --markdown "${output_dir}/alps_full_e_two_way.md"
    rsync -a --partial \
    ...
    "${venv}/bin/python" "${repo_root}/scripts/script_release/internal/summarize_alps_ablation.py" \
      --output-root "${output_dir}" \
      --results "${results}" \
    ...
    "${venv}/bin/python" "${repo_root}/scripts/script_release/internal/summarize_alps_full_matrix.py" \
      --output-root "${output_dir}" \
      --results "${results}" \
