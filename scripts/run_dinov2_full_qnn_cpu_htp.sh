...
output_dir="${OUTPUT_DIR:-/tmp/omnifetch-baselines/dinov2-small-full-qnn}"
...
device_dir="${QNN_DEVICE_DIR:-/data/local/tmp/omnifetch_baselines/dinov2_small_full_qnn}"
...
if ((run_only == 0)); then
  if [[ ! -d "${qnn_py_deps}/onnx" || ! -d "${onnx_py_deps}/onnx" ]]; then
    ...
    echo "Run scripts/run_dinov2_qnn_baseline.sh --prepare-only once first." >&2
    exit 1
  fi
